...
{
	//Set kurang dpd 100000000, boleh rob 1 item

	if (dwBadFame < CServerSystem::LootItemNum)//miningmso 30 //jackmso 2
	{
		return 0;
	}
	//if (dwBadFame < 30)
...
{
	//Set kurang dpd 30, EQ tak kena rob
	if( dwBadFame < CServerSystem::CanBadFame )
		return 0;
	else if(dwBadFame<CServerSystem::CanBadFame+20)
		return 10;
	else if(dwBadFame<CServerSystem::CanBadFame+40)
		return 15;
	else if(dwBadFame<CServerSystem::CanBadFame+60)
		return 20;
	else if(dwBadFame<CServerSystem::CanBadFame+80)
		return 25;
...
void CLootingManager::CloseLootingRoom( DWORD dwDiePlayer, BOOL bMsgToClient )
{
	...
	if( pRoom )
	{
		pRoom->EndLootingRoomMsg( bMsgToClient );
