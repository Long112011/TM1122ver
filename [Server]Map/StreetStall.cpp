...
//	m_nUsable = DEFAULT_USABLE_INVENTORY;	
	...
	m_nTotalGold =  0;  // 2014-10-30 悧

}

// 2014-10-30 

BOOL cStreetStall::FillCell( ITEMBASE* pBase, DWORD money, DWORD gold, BOOL bLock, DWORD Volume, WORD wAbsPosition )
{
	WORD pos = 0;
...
// 2014-10-30  阮초暄?
void cStreetStall::SetGold( WORD pos, DWORD gold )
{
	if( pos > MAX_STREETSTALL_CELLNUM ) return; // 갯수를 초과한다??
	if( !m_sArticles[pos].bFill ) return; // 비워져 있다..
	if( m_sArticles[pos].bLock ) return;
...
void cStreetStall::GetStreetStallInfo( STREETSTALL_INFO& stall )
{
	...
	ITEM_STONE_OPTION_INFO StoneOptionInfo[SLOT_STREETSTALL_NUM];        // 2014-12-15 斤口써뭐竟
	...
	WORD StoneOptionNum=0;   // 斤口鑒좆 2014-12-15
	WORD PetInfoNum = 0;
	WORD TitanInfoNum = 0;
	...
	for(int i=0;i<MAX_STREETSTALL_CELLNUM;++i)
	{
		...
		stall.Item[i].gold = m_sArticles[i].dwGold;  // 2014-10-30 쉥랙箇돕와빵똥
		...
		stall.Item[i].ItemQuality = m_sArticles[i].sItemBase.ItemQuality;
		stall.Item[i].ItemEntry1 = m_sArticles[i].sItemBase.ItemEntry1;
		stall.Item[i].ItemEntry2 = m_sArticles[i].sItemBase.ItemEntry2;
		stall.Item[i].ItemEntry3 = m_sArticles[i].sItemBase.ItemEntry3;
		if( stall.Item[i].Fill )
		{
			...
			ITEM_INFO * pItem =ITEMMGR->GetItemInfo(stall.Item[i].wIconIdx);

			if( ITEMMGR->IsStoneOptionItem(stall.Item[i].dwStoneIdx,(eITEM_KINDBIT) pItem->ItemKind))
			{
				if(stall.Item[i].dwStoneIdx!=0)
				{
					memcpy(&StoneOptionInfo[StoneOptionNum], GetOwner()->GetItemStoneOption(stall.Item[i].dwStoneIdx), sizeof(ITEM_STONE_OPTION_INFO));
				    ++StoneOptionNum;
				}
	...
	{
		stall.AddableInfo.AddInfo( CAddableInfoList::ItemStoneOption, sizeof(ITEM_STONE_OPTION_INFO)*StoneOptionNum,StoneOptionInfo );
		stall.StoneCount = StoneOptionNum;
	}
	if( PetInfoNum )
	{
