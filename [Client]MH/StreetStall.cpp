...
void CStreetStall::ShowBuyStall()
{
	...
	m_pEnterBtn->SetDisable( TRUE );
	m_pImgBtn->SetActive(FALSE);
	m_ImageGrid->SetActive(FALSE);
	m_ImageGrid->SetDisable(TRUE);
	ResetDlgData();
	return;
}
	...
	for( int i=0;i<SLOT_STREETSTALL_NUM;++i)
	{
		...
		if( bDelOption && pIcon )
		{
			...
			if( pItem )
			{
				...
				else if( ITEMMGR->GetItemStoneOption(pItem->GetStoneIdx()))
				{
					ITEMMGR->RemoveItemStoneOption(pItem->GetStoneIdx());
				}
...
BOOL CStreetStall::FakeMoveIcon(LONG x, LONG y, cIcon * icon)
{
	...
	if( (((CItem*)icon)->GetItemBaseInfo()->ItemStatic == ITEM_STATIC_NEWBIE))
	{
...
void CStreetStall::ChangeItemStatus( ITEMBASE* pbase, DWORD money, DWORD gold, BOOL bLock, int nDivideKind )
{
	...
	if( ITEMMGR->IsPetSummonItem( (WORD)pItem->GetData() ) )
	{
		...
		ITEMMGR->SetToolTipIcon( pItem, ITEMMGR->GetItemOption(pItem->GetDurability()), ITEMMGR->GetItemRareOption(pItem->GetRareness()),0,ITEMMGR->GetItemStoneOption(pItem->GetStoneIdx()) ); 
	else
		ITEMMGR->SetToolTipIcon( pItem, NULL, ITEMMGR->GetItemRareOption(pItem->GetRareness()),0,ITEMMGR->GetItemStoneOption(pItem->GetStoneIdx())); 
	if( bLock )
	{
		if( ITEMMGR->IsDupItem((WORD)pItem->GetData()) )
		{
