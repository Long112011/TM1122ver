...
{
	if(m_GXOHandle == NULL||!g_pExecutive)		return;
	//if(m_GXOHandle == NULL)		return;
...
void CEngineObject::Detach(CEngineObject* pChildObject)
{
	...
	if(pChildObject->m_GXOHandle==NULL)
	{
		return;
	}
...
{
	if(m_GXOHandle == NULL||!g_pExecutive)		return;//if(m_GXOHandle == NULL)		return;
...
VOID CEngineObject::SetMaterialIndex(DWORD dwIndex)
{
	...
	// Index狼 裹困啊 逞绢啊歹扼档 郴何俊辑 抗寇贸府甫 窍扁 锭巩俊 酒公繁 捞惑 绝澜. 
	// chr捞唱 chx俊 疙矫等 MTL 颇老狼 鉴辑啊 Index啊 等促. 
