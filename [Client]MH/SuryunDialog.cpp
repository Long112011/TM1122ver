...
void CSuryunDialog::SetAbilityToolTip(CAbilityIcon* pIcon)
{
	...
	if(imgEquipInfo)
	{
		...
		pIcon->SetAutoLineToolTip("",RGBA_MAKE(255, 255, 255, 255),&imgToolTip,46,imgEquipInfo,imgEquipInfo);
	}
	else
	{
		pIcon->SetToolTip( "", RGBA_MAKE(255, 255, 255, 255), &imgToolTip, TTCLR_ITEM_CANEQUIP );
	...
	wsprintf( line, "[%s]", pInfo->GetInfo()->Ability_name );
	...
	if( pInfo->GetUseType() == eAUT_Passive )
	{
		...
		{
			sprintf(tooltip,"         %s",pInfo->GetInfo()->Ability_name);
			sprintf(tooltip2,"                                       %s",CHATMGR->GetChatMsg(571));
			pIcon->AddToolTipLine2(tooltip,tooltip2, TTTC_FREELIMIT,TTTC_BUYPRICE );
		}
		else
		{
			sprintf(tooltip,"         %s",pInfo->GetInfo()->Ability_name);
			sprintf(tooltip2,"                                %s",CHATMGR->GetChatMsg(611));
			pIcon->AddToolTipLine2(tooltip,tooltip2, TTTC_FREELIMIT,TTTC_BUYPRICE );
		}
	}
	else
	{
		sprintf(tooltip,"         %s",pInfo->GetInfo()->Ability_name);
		sprintf(tooltip2,"                                        %s",CHATMGR->GetChatMsg(572));
		pIcon->AddToolTipLine2(tooltip,tooltip2, TTTC_FREELIMIT,TTTC_BUYPRICE );
	}
	...
	if( (int)NeedAbilityExp != -1 )
	{
		wsprintf( tooltip, CHATMGR->GetChatMsg(2470),AbilLevel );
		pIcon->AddToolTipLine( tooltip, RGB_HALF(255,255,200) );
		...
		if(pInfo->GetInfo()->Ability_AcquireKind == 3 && HERO->GetLevel() < pInfo->GetInfo()->Ability_AcquireParam1)
		{
			...
			pIcon->AddToolTipLine( tooltip, TTCLR_ENERMY );
		}
		pIcon->AddToolTipLine("");
		wsprintf( line, CHATMGR->GetChatMsg(2460) );
		pIcon->AddToolTipLine( line, ICONTEXT_DISABLE_BYZEROSUNG );
		pIcon->AddToolTipLine("");
		wsprintf( tooltip, "%s: %d", CHATMGR->GetChatMsg(573), NeedAbilityExp );
		pIcon->AddToolTipLine( tooltip, TTTC_RAREITEM );
	}
	else
	{
