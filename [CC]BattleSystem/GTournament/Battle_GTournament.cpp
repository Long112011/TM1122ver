...
void CBattle_GTournament::OnCreate(BATTLE_INFO_BASE* pCreateInfo, CBattleTeam* pTeam1, CBattleTeam* pTeam2)
{
	...
	m_vTitlePos.x = ((float)GAMERESRCMNGR->m_GameDesc.dispInfo.dwWidth - 512 * m_vTitleScale.x) / 2;
	m_vTitlePos.y = 32;
...
BOOL CBattle_GTournament::Judge()
{
	...
	if( m_GTInfo.m_dwFightTime == 0 )
	{
		...
		else
		{
			...
			if( JudgeMemberLevel() == FALSE )			// 살아남은 사람의 Level의 합이 낮은
			if( JudgeMemberLevelExp() == FALSE )		// 살아남은 사람의 경험치 총합이 낮은
			{
				// Random
				DWORD data = rand()%2;
				Victory( data, 1-data );
