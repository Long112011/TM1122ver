				...
				if( we & WE_MOUSEOVER )
					if( m_ppPushupTabBtn[i]->IsActive() && m_ppPushupTabBtn[i]->PtInWindow( mouseInfo->GetMouseX(), mouseInfo->GetMouseY() ) )
					{
						...
						if( m_bPushTabWithMouseOver )
						{
							...
							{
								SelectTab(i);
							//	CHATMGR->AddMsg(CTC_SYSMSG,"%d m_bSelTabNum:%d",i,m_bSelTabNum);
								m_bSelTabNum = i;
							}
						}
						else
...
void CPetInventoryDlg::OnActionEvent( LONG lId, void *p, DWORD we )
{
	...
	{
		POSTYPE pos = 0;
		if (GetPositionForXYRef(g_UserInput.GetMouse()->GetMouseEventX(),
			g_UserInput.GetMouse()->GetMouseEventY(), pos))
		{
			...
			if( pItem->IsLocked() ) 
			{
			//	CHATMGR->AddMsg(CTC_SYSMSG,"block3");
				return;
			}
...
void CPetInventoryDlg::MoveItemToPetInventory(CItem* pFromItem)
{
//	DWORD wIndex = m_dwValidTabNum - m_bSelTabNum;
//	CHATMGR->AddMsg(CTC_SYSMSG,"%d %d  %d",m_dwValidTabNum,m_bSelTabNum,wIndex);
	...
	{
		for (int i = TP_PETINVEN_START; i < TP_PETINVEN_START + m_bSelTabNum*TABCELL_PETINVEN_NUM + TABCELL_PETINVEN_NUM; i++)
		{
			...
			if (pToItem)
			{
				...
				if( pToItem->IsLocked() ) 
				{
				//	CHATMGR->AddMsg(CTC_SYSMSG,"block1");
					return;
				}
				if (ITEMMGR->IsDupItem(pFromItem->GetItemIdx()))
				if (pToItem->GetItemIdx() == pFromItem->GetItemIdx() && pToItem->GetDurability() < MAX_YOUNGYAKITEM_DUPNUM &&
					pFromItem->GetDurability() < MAX_YOUNGYAKITEM_DUPNUM)
				{
					FakeItemCombine(i, pFromItem, pToItem,TRUE);
					return;
				}
			}
		}
	...
	for (int i = TP_PETINVEN_START + m_bSelTabNum * TABCELL_PETINVEN_NUM; i < TP_PETINVEN_START + m_bSelTabNum * TABCELL_PETINVEN_NUM + TABCELL_PETINVEN_NUM; i++)
	{
		//if(wIndex < 0)
		//	continue;
		pToItem = GetItemForPos(i);
		...
		if( pToItem->IsLocked() ) 
		{
		//	CHATMGR->AddMsg(CTC_SYSMSG,"block2");
			return;
		}
		if (NULL == pToItem)
		{
...
BOOL CPetInventoryDlg::CanBeMoved(CItem* pItem,POSTYPE pos)
{
	...
	if( !(TP_INVENTORY_START <= fromPos && fromPos < TP_INVENTORY_END) &&
		!(TP_SHOPINVEN_START <= fromPos && fromPos < TP_SHOPINVEN_END/*-40*/) &&
	...
	else if(TP_SHOPINVEN_START <= pos && pos < TP_SHOPINVEN_END/*-40*/)
	{
		if( TP_PETWEAR_START <= fromPos && fromPos < TP_PETWEAR_END )
			return TRUE;
		else
...
BOOL CPetInventoryDlg::FakeGeneralItemMove( POSTYPE ToPos, CItem * pFromItem, CItem * pToItem)
{
	...
	//	return FALSE;
	if( pFromItem->GetItemParam() & ITEM_PARAM_SEAL && !(pFromItem->GetItemKind() & eEQUIP_ITEM) )
		return FALSE;
...
BOOL CPetInventoryDlg::FakeItemCombine(POSTYPE ToPos, CItem* pFromItem, CItem* pToItem, BOOL bIsR)
{
	if( pFromItem->GetPosition() == pToItem->GetPosition() )
		return FALSE;
	...
	if( !bIsR&&pToItem && CanBeMoved( pToItem, pFromItem->GetPosition() ) == FALSE )
		return FALSE;
