...
void CPet::Release()
{
	if (m_pMaster)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			if (this == pPetMgr->GetCurSummonPet(i))
			{
				pPetMgr->SetCurSummonPetNull(i);
			}
		}
	}
...
void CPet::AddFriendship(int friendship, BOOL bSendMsg/* = FALSE*/)
{
	...
	{
		CPetManager* pPetMgr = m_pMaster->GetPetManager();
		if (pPetMgr->CheckCurSummonPetKindIs(this, ePK_EventPet))
			return;
	}
...
//	else
//	{
//		//to do 장착 아이템과 펫 종류에 따라 게이지 충? 속도 차이.
//	}
//}
...
void CPet::SendPetInfoMsg()
{
	...
	msg.dwObjectID = m_pMaster->GetID();
	msg.dwData1 = m_PetTotalInfo.PetSummonItemDBIdx;
