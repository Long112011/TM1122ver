...
/*
//SW0508 ±¹³»¿Í ´Þ¸® JAPAN Edition ¿¡¼­´Â ¸í¼º°ú º°È£°¡ ¾ø´Ù.
*/
...
void CCharacterDialog::Linking()
{
	...
	m_ppStatic.fame = (cStatic *)GetWindowForID(CI_CHARFAME);	// ¸í¼º
	m_ppStatic.spname = (cStatic *)GetWindowForID(CI_SPCHARNAME);	// º°È£
#else
	...
	m_ppStatic.stage = (cStatic *)GetWindowForID(CI_CHARSTAGE);		// °æÁö

//	m_ppStatic.job = (cStatic *)GetWindowForID(CI_CHARJOB);		// Á÷¾÷
//	m_ppStatic.ideology = (cStatic *)GetWindowForID(CI_CHARIDEOLOGY);	// °¡Ä¡°ü
	...
	m_ppStatic.point = (cStatic *)GetWindowForID(CI_CHARPOINT);		// ÀÜ¿©Æ÷ÀÎÆ®
	
	m_ppStatic.meleeattack = (cStatic *)GetWindowForID(CI_CHARATTACK);	// °ø°Ý·Â
	m_ppStatic.rangeattack = (cStatic *)GetWindowForID(CI_LONGATTACK);	// °ø°Ý·Â
...
//	m_ppStatic.move = (cStatic *)GetWindowForID(CI_CHARMOVE);
	...
	m_MoveSpeed = (cStatic*)GetWindowForID(CI_MOVESPEED);	//ÒÆ¶¯ËÙ¶È
...
void CCharacterDialog::UpdateData()//¸üÐÂÊôÐÔÁÐ±í¹¦ÄÜ
{
...
void CCharacterDialog::UpdateMoveSpeedDisplay()
{
	...
	m_MoveSpeed->SetStaticText(buf);  // Õâ¸ö¿Ø¼þ±ØÐëÊÇ´¿ÎÄ±¾£¬²»ÊÇ½ø¶ÈÌõÀàÐÍ£¡
}
...
void CCharacterDialog::RefreshGuildInfo()
{
	...
	{
		m_pMunpaMarkDlg->SetActive(FALSE); //¾ø¾Ö¾ßÁö...
