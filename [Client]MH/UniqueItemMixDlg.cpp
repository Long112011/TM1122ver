...
CUniqueItemMixDlg::CUniqueItemMixDlg()
{
	...
	cImage imgTooltip;
	SCRIPTMGR->GetImage(244, &imgTooltip, PFT_HARDPATH);
...
void CUniqueItemMixDlg::AddUniqueItem(CItem* pItem)
{
	...
	{
		ITEMMGR->SetToolTipIcon((cIcon*)&m_VirtualItem, ITEMMGR->GetItemOption(pItem->GetDurability()), ITEMMGR->GetItemRareOption(pItem->GetRareness()),0,ITEMMGR->GetItemStoneOption(pItem->GetStoneIdx()));  // 2014-11-15 맒
	}
	else
	{
		ITEMMGR->SetToolTipIcon((cIcon*)&m_VirtualItem, NULL, ITEMMGR->GetItemRareOption(pItem->GetRareness()),0,ITEMMGR->GetItemStoneOption(pItem->GetStoneIdx())); // 2014-11-15 맒
	}
