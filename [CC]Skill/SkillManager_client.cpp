...
void CSkillManager::ReLoadSkillInfo()
{
	...
	g_PoolSTLIST.Release();
	g_PoolSTLIST.Init(100, 50, "g_PoolSTLIST");
	//m_SkillInfoTable.Initialize(2000);//kiv
	//m_SkillObjectTable.Initialize(512);//kiv
	LoadSkillInfoList();
...
void CSkillManager::LoadSkillChangeInfoList()
{
	...
	if (file.IsInited() == FALSE)
	{
		//ASSERTMSG(0, "SkillChangeList甫 肺靛窍瘤 给沁嚼聪促.");
		return;
	}
	while (1)
	{
...
void CSkillManager::LoadStateList()
{
	...
	if (file.IsInited() == FALSE)
	{
		//ASSERTMSG(0, "StateList甫 肺靛窍瘤 给沁嚼聪促.");
		return;
	}
	while (1)
	{
...
BOOL CSkillManager::ExcuteSkillSYN(CHero* pHero, CActionTarget* pTarget, CSkillInfo* pSkillInfo)
{
	...
	{
		if (pSkillInfo->GetSkillIndex() == 2013 && STATUSICONDLG->CheckStatusIcon(eStatusIcon_PhyAttackUp))
			return FALSE;
	...
	else
	{
		if (pHero->IsAutoAttacking())
		{
			...
			else
			{
				...
				OBJECTSTATEMGR->EndObjectState(pHero, eObjectState_SkillDelay,GetComboDelayTime(pHero->GetWeaponEquipType()) );//300
				pHero->SetCurComboNum(0);
			}
		}
	}
	...
	if (pHero->InTitan() == TRUE)
	{
		if (SkillInfo->SkillKind == SKILLKIND_TITAN)
		{
			if (SkillInfo->WeaponKind != 0)
			{
				...
				{
					CHATMGR->AddMsgFade(0xFF0000, CHATMGR->GetChatMsg(2607));
					pHero->DisableAutoAttack();
					return FALSE;
				}
