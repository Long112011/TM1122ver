...
void CPlayer::SetBaseMotion()
{
	...
	else
	{
		...
		if (WeaponEquipType == WP_AXE)
			...
			m_StandardMotion = eMotion_Battle_Gwun_Standard;  // ¼ÙÉè910ÊÇ´Ì¿ÍË«µ¶µÄ±ê×¼´ý»ú¶¯×÷
		else
			m_StandardMotion = eMotion_Battle_Gum_Standard + WeaponEquipType - 1;
		...
		if (m_MoveInfo.MoveMode == eMoveMode_Walk)
		{
			if (WeaponEquipType == WP_AXE)
			{
				m_Move_Start_Motion = eMotion_Battle_Do_Walk; // ¸«Í·²½ÐÐ¶¯×÷
		...
		else
		{
			if (WeaponEquipType == WP_AXE)
			{
				m_Move_Start_Motion = eMotion_Battle_Do_Run; // ¸«Í·ÅÜ¶¯
...
#ifdef  _MUTIPET_
void CPlayer::SetPet(BYTE ID, CPet* pet)//µ¶¸ç  3pet
{
	for (int i = 0; i < 3; ++i)
	{
		if (m_pPet[i] == pet)
...
void CPlayer::RemovePet(CPet* pet)//µ¶¸ç  3pet
{
	for (int i = 0; i < 3; ++i)
	{
		if (m_pPet[i] == pet)
