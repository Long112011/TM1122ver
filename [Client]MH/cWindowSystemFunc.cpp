	...
	if(srcDlg->GetType() & WT_ICON)
	{
		...
		else
		{
			if( pIcon->CanDelete() )
			{
				...
				// new changelook dialog
				if (srcDlg->GetType() == WT_CHANGELOOK_ITEM)
				{
					GAMEIN->GetChangeLookDialog()->ReleaseIconWear((cIcon*)srcDlg);	//Daily 增加狩猎生命物品释放
