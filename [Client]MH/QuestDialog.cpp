...
#define TOOLTIPLENG MAX_ITEMDESC_LINE+26

extern char JACKJACK[];
//////////////////////////////////////////////////////////////////////
...
{	
	...
	//
	for(int i=0; i<m_pIconGridDlg->GetCellNum(); i++)	
...
//		if(pSQuest->Pos == Index)
		if(dwCount == Index)
		{
			...
			pSQuest->pIcon->SetHeaderImage(&imgHeader);
			cImage imgToolTip;
			SCRIPTMGR->GetImage(26, &imgToolTip, PFT_JACKPATH );
			pSQuest->pIcon->SetToolModel(1);

			//cImage Image;
			//SCRIPTMGR->GetImage(244, &Image, PFT_HARDPATH);
		//	pSQuest->pIcon->SetToolTip( "", RGBA_MAKE(255, 255, 255, 255), &Image, TTCLR_ITEM_CANEQUIP );
			cImage * imgEquipInfo;
			imgEquipInfo=pSQuest->pIcon->GetBasicImage();
			if(imgEquipInfo)
			{
				//pIcon->AddToolTipImageEquip(imgEquipInfo);
				pSQuest->pIcon->SetAutoLineToolTip("",RGBA_MAKE(255, 255, 255, 255),&imgToolTip,46,imgEquipInfo,imgEquipInfo);
			}

			wsprintf(buf, "[%s]", pSQuest->pQuestItemInfo->ItemName);
			pSQuest->pIcon->AddToolTipLine(buf, TTTC_DEFAULT,NULL,6);
			pSQuest->pIcon->AddToolTipLine("");
			pSQuest->pIcon->AddToolTipLine(JACKJACK);
			pSQuest->pIcon->AddToolTipLine("");
			pSQuest->pIcon->AddToolTipLine("");
			pSQuest->pIcon->AddToolTipLine("");
...
{
	if(strlen(pString) > TOOLTIPLENG)
	{
		char* pStr = pString;
		char tb[TOOLTIPLENG+2] = { 0,  };
		int len = 0;
		...
		{						
			...
			if(strlen(tb) >= TOOLTIPLENG)
			{
				pIcon->AddToolTipLine(tb, TTTC_DESCRIPTION);
				memset(tb, 0, TOOLTIPLENG+2);
				len = 0;
			}
		}
		if(len)
