...
void CObjectActionManager::Die(CObject* pObject,CObject* pAttacker,BOOL bFatalDamage,BOOL bCritical, BOOL bDecisive)
{
	...
	HERO->ClearTarget(pObject);
	if(OBJECTMGR->GetSelectedObject() == pObject)
	{
...
{
	if(pObject->GetObjectKind()==eObjectKind_Monster)//kiv
		int i = 0;//kiv
	pObject->Damage(pAttacker,DamageKind,Damage,ShieldDamage,bCritical, bDecisive, 0/*titanObserbDamage*/);
}
...
void CObjectActionManager::ApplyResult(CObject* pObject,CObject* pOperator,RESULTINFO* pRInfo,BYTE DamageKind)
{
	...
	{
		Damage(pObject,pOperator,DamageKind,pRInfo->RealDamage,pRInfo->ShieldDamage,pRInfo->bCritical,pRInfo->bDecisive,0/*pRInfo->TitanObsorbDamage*/);
		if(pObject->IsDied())
			Die(pObject,pOperator,FALSE,pRInfo->bCritical,pRInfo->bDecisive);
	}
	if(pRInfo->CounterDamage)
	{
		Damage(pOperator,pObject,eDamageKind_Counter,pRInfo->CounterDamage,0,0,0,0/*pRInfo->TitanObsorbDamage*/);
		if(pOperator->IsDied())
			Die(pOperator,pObject,FALSE,FALSE,FALSE);
	}
	//if(pRInfo->HealLife)//kiv
	//	Heal(pObject,pOperator,0,pRInfo->HealLife);//kiv
	if (pRInfo->HealLife)
	{
		if (pObject->GetObjectKind() == eObjectKind_Player&&pObject->GetID() == HEROID)
			HERO->Heal(pObject, 0, pRInfo->HealLife);
		else
