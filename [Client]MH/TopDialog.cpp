...
void CTopDialog::Linking()
{
	pDlg = WINDOWMGR->GetWindowForID(RANKING_DLG);
	pUpdateTime = new cStatic;
	pUpdateTime->Init(410, 5, 0, 0, NULL, TOP_UPDATE_TIME);
	...
	for (int i = 0; i<7; i++)
	{
		pColumnStatic[i] = (cStatic*)GetWindowForID(RK_STC_RANK_NUM + i);
	}
	LoadList();
	LoadHeroList();
	for (int i = 0; i<5; i++)
	{
		pPageList[i] = ((cPushupButton*)pDlg->GetWindowForID(RK_BTN_PAGE01 + i));
	}
}
//int h = 1;
DWORD CTopDialog::ActionEvent(CMouse* mouseInfo)
{
	...
	//if (we & WE_MOUSEOVER)
	//{
		//if (pHeroList->PtInWindow)
		//if (!WINDOWMGR->IsMouseDownUsed())
		//if (pHeroList->PtIdxInRow(mouseInfo->GetMouseEventX(), mouseInfo->GetMouseEventY()))
	//}
	if (we & WE_RBTNCLICK)
	{
...
bool CTopDialog::checkName(char * c)
{
...
void CTopDialog::OnActionEvent(LONG lId, void * p, DWORD we)
{
	if (we & WE_PUSHDOWN)
	{
		if (RK_BTN_PAGE01 <= lId && lId <= RK_BTN_PAGE05)
		{
			for (int i = 0; i<5; i++)
			{
				pPageList[i]->SetPush(false);
			}
			m_TableIdx = lId - RK_BTN_PAGE01;
	...
	else if (we & WE_PUSHUP)
	{
		if (RK_BTN_PAGE01 <= lId && lId <= RK_BTN_PAGE05)
		{
			pPageList[lId - RK_BTN_PAGE01]->SetPush(true);
		}
	}
}
void CTopDialog::SetHeroTopListInfo(SEND_TOPLIST * pListInfo)
{
...
void CTopDialog::GetHeroTopListInfo()
{
	MSG_DWORD msg;
	msg.Category = MP_USERCONN;
	msg.Protocol = MP_USERCONN_HERO_TOPLIST_SYN;
	msg.dwObjectID = HEROID;
	msg.dwData = 1;
	NETWORK->Send(&msg, sizeof(msg));
}
void CTopDialog::LoadList()
{
...
void CTopDialog::SetHeroPage(int PageIndex)
{
	int Count = pHeroData.wCount;
	int PageNum = Count / PAGELISTNUM;
	int PageNext = Count%PAGELISTNUM;
...
void CTopDialog::SetPage(int PageIndex)
{
	SetHeroPage(PageIndex);
	ShowAllList();
}
void CTopDialog::OpenLastTab()
{
	if (m_TableIdx != 0)
	{
		pPageList[m_TableIdx]->SetPush(true);
