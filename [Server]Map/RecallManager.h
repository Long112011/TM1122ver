...
//	void Recall(CObject * pRecaller, DWORD MonsterKind, WORD DropItemID, VECTOR3* pPos, BOOL bBoss);
	...
	CMonster* InsDungeonRecall(WORD dwObjectKind, WORD wMonsterKind, DWORD dwGroupID, DWORD dwChannel, VECTOR3* pMonsterPos, WORD wRadius);
	void DeleteInsDungeonRecallMonster(DWORD dwChannel);
	CMonster* InsDungeonMissionRecall(WORD wMonsterKind, BYTE bMonsterCount, DWORD dwChannel, VECTOR3* pMonsterPos, WORD wRadius);
};
