...
enum eWeekData{ eWeekforGT = 1, eWeekforSG = 2, };
...
class CObjectManager
{
	...
	void GetTargetInRange(VECTOR3* pPivotPos, float Radius, ObjectArray<LPOBJECT>* pObjectArray, BOOL bPK, BOOL pBehindWall, BOOL pIgnoreBosses);
	void GetTargetInRangeForAI(VECTOR3* pPivotPos, float Radius, ObjectArray<LPOBJECT>* pObjectArray, BOOL bPK);
	...
	void SetHideWingOption(DWORD dwOption);
	void ApplyHideWingOptionToAll();
	void ApplyFortWarCustomName();
	...
	DWORD GetNearMonsterByName(const char* name);

	float GetDistance(CObject*, CObject*);
	float GetDistance(CObject* b);
	private:
	cPtrList m_listpAutoTarget;
	cPtrList m_listpAutoFriend;

	BOOL CompareAutoObject(CObject* p_pObject1, CObject* p_pObject2);
	public:

	void UpdateAutoTarget(float fDistances);




	void ClearAutoTarget(void);
	void UpdateAutoTarget(void);
	cPtrList* GetAutoTargetList(void) { return &m_listpAutoTarget; }




	void ClearAutoFriend(void);
	void UpdateAutoFriend(void);
	cPtrList* GetAutoFriendList(void) { return &m_listpAutoFriend; }
	BOOL IsPetHiding(){ return m_bHidePetFlg; }
	BOOL IsNpcHiding(){ return m_bHideNpcFlg; }
	BOOL IsMonsterHiding(){ return m_bHideMonsterFlg; }
};
