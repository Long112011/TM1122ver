...
void CShopItemManager::CheckEndTime(BOOL IsCheck)
{
	...
	while (pShopItem = m_UsingItemTable.GetData())
	{
		...
		if (pItemInfo->SellPrice == eShopItemUseParam_Realtime)
		{
			...
			else
			{
				...
				if (!checksum.GetYear() && !checksum.GetMonth() && !checksum.GetDay() && !checksum.GetHour() && !checksum.GetMinute())
				 ...
				 (
				 pItemInfo->SellPrice == eShopItemuseParam_FBItem &&
				 DungeonMGR->IsDungeon(m_pPlayer->GetCurMapNum()) &&
				 ...
				 ))
		{
			...
			{
				if (!DungeonMGR->IsDungeon(m_pPlayer->GetCurMapNum()) && pItemInfo->ItemKind == eSHOP_ITEM_GETIN_ITEM &&
					DungeonMGR->GetFubenKey(m_pPlayer->GetMapChangePoint()) == 1)
					continue;
...
void CShopItemManager::UpdateLogoutToDB()
{
	...
	while (pShopItem = m_UsingItemTable.GetData())
	{
		...
		if (pItemInfo->SellPrice == eShopItemuseParam_FBItem)
		{
			if (DungeonMGR->IsDungeon(m_pPlayer->GetCurMapNum()))
			{
				DWORD checktime = gCurTime - pShopItem->LastCheckTime;
				if (checktime > 30000)			checktime = 30000;
				if (checktime >= pShopItem->ShopItem.Remaintime)
...
BOOL CShopItemManager::CalcShopItemOption(DWORD wIdx, BOOL bAdd, DWORD Param)
{
	...
	if (pItemInfo->ItemKind == eSHOP_ITEM_INCANTATION)
	{
		...
		{
			m_pPlayer->SetExtraMugongSlot(m_pPlayer->GetExtraMugongSlot() + 1);
		}
#endif
	...
	else if (pItemInfo->ItemKind == eSHOP_ITEM_HERB)
	{
		if (pItemInfo->Life > 0)
		{
			pShopItemOption->Life += (DWORD)(pItemInfo->Life*calc);
			if (pShopItemOption->Life <= 0)
				pShopItemOption->Life = 0;
			if (bAdd == FALSE)
				STATSMGR->CalcCharLife(m_pPlayer);
		...
		if (pItemInfo->Shield > 0)
		{
			pShopItemOption->Shield += (DWORD)(pItemInfo->Shield*calc);
			if (pShopItemOption->Shield <= 0)
				pShopItemOption->Shield = 0;
			if (bAdd == FALSE)
				STATSMGR->CalcCharShield(m_pPlayer);
		...
		if (pItemInfo->NaeRyuk > 0)
		{
			pShopItemOption->Naeryuk += (DWORD)(pItemInfo->NaeRyuk*calc);
			if (pShopItemOption->Naeryuk <= 0)
				pShopItemOption->Naeryuk = 0;
			if (bAdd == FALSE)
				STATSMGR->CalcCharNaeruyk(m_pPlayer);
...
BOOL CShopItemManager::TakeOffAvatarItem(WORD ItemIdx, WORD ItemPos, BOOL bCalcStats)
{
	...
	PACKEDDATA_OBJ->QuickSend(m_pPlayer, &msg, sizeof(msg));
	CalcAvatarOption(bCalcStats);
	if (pItemInfo->ItemKind == eSHOP_ITEM_IMAGENAME)
	{
		if (pAvatar[8] == 0)
			m_pPlayer->SetImageName(0);
	}
	return TRUE;
}
void CShopItemManager::CalcAvatarOption(BOOL bCalcStats)
{
	...
	for (int i = 0; i<eAvatar_Max; i++)
	{
		...
		if (!pItemInfo)		continue;
		if (pItemInfo->GenGol > 0)
			pAvatarOption->Gengol += (DWORD)pItemInfo->GenGol;
		if (pItemInfo->MinChub > 0)
			pAvatarOption->Minchub += (DWORD)pItemInfo->MinChub;
		if (pItemInfo->CheRyuk > 0)
			pAvatarOption->Cheryuk += (DWORD)pItemInfo->CheRyuk;
		if (pItemInfo->SimMek > 0)
			pAvatarOption->Simmek += (DWORD)pItemInfo->SimMek;

		if (pItemInfo->Life > 0)
			pAvatarOption->Life += (DWORD)pItemInfo->Life;
		if (pItemInfo->Shield > 0)
			pAvatarOption->Shield += (DWORD)pItemInfo->Shield;
		if (pItemInfo->NaeRyuk > 0)
			pAvatarOption->Naeruyk += (DWORD)pItemInfo->NaeRyuk;
...
DWORD CShopItemManager::PutSkinSelectLook(MSG_DWORD7* pMsg)
{
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Mask] = pMsg->dwData0;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Hat] = pMsg->dwData1;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Sticker] = pMsg->dwData2;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Weapon] = pMsg->dwData3;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Dress] = pMsg->dwData4;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Shoulder] = pMsg->dwData5;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Shoes] = pMsg->dwData6;

	return eSkinResult_Success;
}
DWORD CShopItemManager::PutSkinSelectItemNew(MSG_DWORD7_SKIN pMsg)
{
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Hat]			= pMsg.dwSkinData[eSkinItem_Hat];
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Dress]		= pMsg.dwSkinData[eSkinItem_Dress];
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Shoes]		= pMsg.dwSkinData[eSkinItem_Shoes];
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Mask]		= pMsg.dwSkinData[eSkinItem_Mask];
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Shoulder]	= pMsg.dwSkinData[eSkinItem_Shoulder];
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Weapon]		= pMsg.dwSkinData[eSkinItem_Weapon];
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Sticker]		= pMsg.dwSkinData[eSkinItem_Sticker];

	/*m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Hat] = pMsg->dwData0;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Dress] = pMsg->dwData1;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Shoes] = pMsg->dwData2;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Mask] = pMsg->dwData3;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Shoulder] = pMsg->dwData4;
	m_pPlayer->GetShopItemStats()->wSkinItem[eSkinItem_Weapon] = pMsg->dwData5;*/
...
int CShopItemManager::UseShopItem(ITEMBASE* pItemBase, SHOPITEMUSEBASE UseBaseInfo, SHOPITEMBASE* pShopItem)
{
	...
	DWORD dwCheckBit = eBossMap | eSiegeWarMap | eQuestRoom | eEventMap | eSurvivalMap | eInsDungeon;
	if (pItemBase->wIconIdx == eIncantation_MemoryStone &&
		g_pServerSystem->GetMap()->IsMapKind(dwCheckBit) &&
		m_pPlayer->GetUserLevel() == eUSERLEVEL_USER)
		return eItemUseErr_Err;
	...
	else if (pItemInfo->ItemKind == eSHOP_ITEM_INCANTATION)
	{
		if (pItemInfo->ItemType == 10)
		{
			...
			{
				if (m_pPlayer->GetExtraMugongSlot() < 7)
				{
...
#define BASIC_STATUS	12
					 ...
					 pItemInfo->ItemIdx == eIncantation_StatusPointReset_1_99)
			{
				...
				m_pPlayer->SetCheRyuk((BASIC_STATUS)+((m_pPlayer->GetReSet() + 1)*GAMERESRCMNGR->GetRebornSet(REBIRTH_CON)) - GAMERESRCMNGR->GetRebornSet(REBIRTH_CON));
				...
				nLevelStatus = nLevelStatus + 0 + (m_pPlayer->GetTurnOne()*GAMERESRCMNGR->GetRebornSet(STAGE_ONE_ATTRI)) + (m_pPlayer->GetTurnTwo()*GAMERESRCMNGR->GetRebornSet(STAGE_TWO_ATTRI)) + (m_pPlayer->GetTurnThree()*GAMERESRCMNGR->GetRebornSet(STAGE_THREE_ATTRI));
...
BOOL CShopItemManager::IsCanInPutFBMap()
{
...
CYHHashTable<SHOPITEMWITHTIME> * CShopItemManager::GetUsingItemInfoAll()
{
