	...
	case MP_SIEGEWAR_UPGRADE_GATE_NACK:
		{
			...
			case 1:
			case 2:
				CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(1676) ); //성주만 성문을 강화할 수 있습니다.
				break;
			case 3:
				CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(117) ); //돈이 부족합니다.	
				break;
			case 4:
				CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(1677) ); //성문 강화에 실패하였습니다.
				break;
			case 5:
				CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(1678) ); //성문을 강화할 수 있는 기간이 아닙니다.
				break;
			}
#else
			switch( pmsg->dwData )
