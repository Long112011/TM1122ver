...
void COptionManager::SetDefaultOption()
{
	...
	m_GameOption.bShowPet			= FALSE;
	m_GameOption.bShowNpc			= FALSE;
	m_GameOption.bShowMonster		= FALSE;
	m_GameOption.bVerticalSync		= FALSE;
	m_GameOption.bFPSChange		= TRUE;
...
//#ifndef _JSKEN_
	if(m_GameOption.bGamma)
	MyCameraEngine->ResetCamera(MAP->GetMapDefaultSight(), DEGTORAD(m_GameOption.nGamma));
//#endif
}
...
void COptionManager::InitForGameIn()
{
	...
	m_saveVsyncStatus = m_GameOption.bVerticalSync;// ((cCheckBox*)pDlg->GetWindowForID(OTI_CB_VSYNC))->IsChecked();

	m_saveFPSStatus = m_GameOption.bFPSChange;
