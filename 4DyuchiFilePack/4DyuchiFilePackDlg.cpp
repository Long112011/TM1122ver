...
DWORD __stdcall MHErrorHandleProc(ERROR_TYPE type,DWORD dwErrorPriority,void* pCodeAddress,char* szStr)
{
	...
	{
		ASSERT_VALID(0,szStr ? szStr : "?");//ASSERTMSG(0,szStr);
	}
	return 0;
}
...
BOOL CMy4DyuchiFilePackDlg::OnInitDialog()
{
	...
	if (NULL != g_hExecutiveHandle)
	{
		...
		if (NULL != pFunc)
		{
			...
			m_pStorage->InitializeFileStorageWithoutRegistry("SS3DFileStorage.dll",40000,10240,MAX_PATH,FILE_ACCESS_METHOD_ONLY_FILE,NULL,0);
			//g_pExecutive->InitializeFileStorage(40000,10240,MAX_PATH,FILE_ACCESS_METHOD_ONLY_FILE,NULL,0);
	...
	//m_pStorage->Initialize(MAX_FILE_ATOM_NUM,1024,64,FILE_ACCESS_METHOD_ONLY_FILE);
	m_pStorage->Initialize(40000,10240,MAX_PATH,FILE_ACCESS_METHOD_ONLY_FILE);
