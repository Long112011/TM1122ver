...
void CPyoGukManager::SendPyogukItemInfo(CPlayer* pPlayer)
{
	...
	ITEM_STONE_OPTION_INFO StoneOptionInfo[SLOT_PYOGUK_NUM];   // 2015-01-04  ˝æ›∏Ωº”≤÷ø‚
	...
	WORD StoneOptionNum=0;   //  ˝æ›À˜“˝ 2015-01-06
	WORD PetInfoNum = 0;
	WORD TitanInfoNum = 0;
	...
	for(int i=0;i<SLOT_PYOGUK_NUM;++i)
	{
		if( pSlot->IsEmpty(StartPos+i) == FALSE )
		{
			...
			ITEM_INFO * pItemInfo = ITEMMGR->GetItemInfo(PyogukItem.PyogukItem[i].wIconIdx);  // 2015-01-06  ˝æ›∏Ωº”µΩ≤÷ø‚

			if(ITEMMGR->IsStoneOptionItem(PyogukItem.PyogukItem[i].wIconIdx, (eITEM_KINDBIT)pItemInfo->ItemKind))
		    {
				if(pPlayer->GetItemStoneOption(PyogukItem.PyogukItem[i].StoneIdx))
				{
					memcpy(&StoneOptionInfo[StoneOptionNum], pPlayer->GetItemStoneOption(PyogukItem.PyogukItem[i].StoneIdx), sizeof(ITEM_STONE_OPTION_INFO));
					++StoneOptionNum;
				}
		    }
	...
	if(StoneOptionNum)
	{
		PyogukItem.AddableInfo.AddInfo( CAddableInfoList::ItemStoneOption, sizeof(ITEM_STONE_OPTION_INFO)*StoneOptionNum, StoneOptionInfo );
		PyogukItem.wStoneOptionCount = StoneOptionNum;
	}
	if( PetInfoNum )
	{
...
*/
		...
		if(pPlayer->GetPyogukNum() == 0) //√≥¿Ω ªÚ¿∏∏È ±◊≥… √ ±‚»≠
		{
...
{
	MONEYTYPE pyogukmon = pPlayer->GetMoney(eItemTable_Pyoguk);
	MONEYTYPE maxmon = pPlayer->GetMaxPurseMoney(eItemTable_Inventory); //¿Œ∫•≈‰∏Æ∑Œ µ∑ ø≈±‚¥œ±Ó.
	
	if( pyogukmon < getMoney)
	{
		getMoney = pyogukmon;
	}
	//2007. 8. 30. CBH - «•±π √÷¥Î±›æ◊ ø¿πˆ«√∑ŒøÏ ∞≥º±
