...
void CBootManager::AddSelfBootList(WORD MyServerKind, WORD MyServerNum,  CServerTable * pServerTable)
{
	...
	//sprintf(filename,"serverset/4/serverlist.msl",4);
	SERVERLISTMGR->Load("serverset/4/serverlist.msl");
	/*if (!SERVERLISTMGR->LoadFromText(filename))
	{
		sprintf(filename, "serverset/%d/serverlist.msl", 4);
	...
	//sprintf(filename,"serverset/4/serverlist_Public.msl",4);
	SERVERLISTMGR->LoadPublicIP("serverset/4/serverlist_Public.msl");
...
#ifdef _DISTRIBUTESERVER_
	//g_BuddyAuth.ConnectToBuddyAuthServerAll();  // 2014-12-28 Distrubute 启动取消, 此处无实际作用!
#endif
}
...
void CBootManager::NetworkMsgParse(DWORD dwConnectionIndex, char* pMsg, DWORD dwLength)
{
	...
	case MP_POWERUP_CONNECT_SYN:
		{
			...
			//2008. 1. 2. CBH - 甘辑滚啊 Agent俊 嘿菌阑锭 敲矾胶 鸥烙 沥焊甫 眉农窍咯 焊辰促.
#ifdef _AGENTSERVER			
	...
	case MP_POWERUP_CONNECT_ACK:
		{
			//dwConnectionIndex 悸泼 for Key(ServerPort)
