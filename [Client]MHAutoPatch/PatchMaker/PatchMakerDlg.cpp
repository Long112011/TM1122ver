...
void CPatchMakerDlg::OnBnClickedButton1()
{
	...
	if(lp && SHGetPathFromIDList(lp, szPath))   
	{
		...
		CWinThread* pThread = AfxBeginThread(GetFileInfo,NULL,THREAD_PRIORITY_NORMAL);
	}
	else   
...
UINT GetFileInfo(LPVOID pParam)
{
	...
	for (size_t i = 0;i<file.size();i++)
	{
		...
		strPath=(file[i].replace(file[i].begin(),file[i].begin()+strlen(sSelectPath),"")).c_str();
