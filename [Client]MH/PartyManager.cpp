	...
	case MP_PARTY_MEMBER_LOGIN: 
		{
			...
			{
				if(pmsg->MemberInfo.LifePercent)			// 새로 접한 파티원
...
void CPartyManager::PartyError(DWORD dwkind)
{
	...
	case eErr_Request_PartyExistence:
		CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(1732) );	//방파에 해있다
		break;
	...
	case eErr_Request_NotMaster:
		CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(1730) );	//방파장이 비접
		break;
