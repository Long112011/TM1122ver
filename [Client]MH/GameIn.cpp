...
#include "StatsCalcManager.h"//刀哥  3pet
#endif //  _MUTIPET_
#ifdef _TESTCLIENT_
#include "AppearanceManager.h"
...
void CGameIn::UserConn_NetworkMsgParse(BYTE Protocol,void* pMsg)
{
	...
	case MP_USERCONN_PET_DIE:
	{
		...
		if (PETMGR->IsSummonPet(pPet))//刀哥 3pet
		{
			...
			PETMGR->SetPetGuageText(pPet,/*pInfo->PetStamina*/0, pInfo->PetFriendly);//刀哥  3pet
	...
	case MP_USERCONN_PET_ADD:
		{
			...
			if (pPlayer)
			{
				pPlayer->SetPet(pPlayer->GetPetId(), pPet);//刀哥  3pet
				pPet->SetMaster(pPlayer);
				...
				if (HEROID == pmsg->MasterID)
				{
					...
					//					PETMGR->SetPetStateDlgInfo(pPet);//刀哥  3pet  屏蔽
