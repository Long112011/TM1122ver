...
public:
	...
	CPet* GetCurSummonPet(BYTE i) { return m_pCurSummonPet[i]; }
	void		SetCurSummonPetNull(BYTE i) { m_pCurSummonPet[i] = NULL; }
	BOOL	IsCurPetFull();
	void		SummonPet(DWORD dwItemDBIdx, BOOL bSummonFromUser = TRUE);
	BOOL		CheckBeforeSummonPet(DWORD dwItemDBIdx);
	void		SummonEventPet();
	void		SetCurSummonPetKind(CPet* pPet, WORD kind);
	BOOL        CheckCurSummonPetKindIs(CPet* pPet, int kind);
	void		CheckEventPetSummonRemainTime(CPet* pPet);

	void		SealPet(CPet* pPet);

	//	void		ExchangePet(DWORD dwItemDBIdx, CPlayer* pNewMaster, BOOL bReduceFriendship = TRUE );
	void		ExchangePet(DWORD dwItemDBIdx, CPlayer* pNewMaster, BOOL bReduceFriendship = TRUE, BOOL bBusiness = FALSE);//刀哥 交易所
	BOOL		IsCurPetSummonItem(DWORD dwItemDBIdx);
	//	void		FeedUpPet(DWORD dwFeedAmount);

	void		PetProcess();

	void		CheckPosValidDistWithMaster(CPet* pPet);

	//	void		CheckStaminaZero();

	void		CalcPetSkillRecharge(CPet* pPet);
	BOOL		IsSkillGuageFull() { return m_bSkillGuageFull; }

	void		ReleaseMoveWithDelay(DWORD delayTime);
	void		CheckDelayRelease(CPet* pPet);

	void		NetworkMsgParse(WORD Protocol, void* pMsg);

	void		UpdateCurPetInfo(CPet* pPet, BYTE	bSummonning);
	void		UpdateLogoutToDB();

	//	void		SetSommonPetStamina(BYTE bFlag);
	int			SetSommonPetFriendship(CPet* pPet, DWORD dwFriendship, BYTE	bSummonning);
	void		SetSommonPetSkillReady(CPet* pPet);

	void		SetPetBuffInfo();
	void		GetPetBuffResultRt(int BuffKind, void* Data);
	DWORD       GetPetBuffValue(int BuffKind);
...
ePB_ReduceCriticalRate = 10,
ePB_ReduceDemageRate = 11,
ePB_ReduceCriticalDmgPlayer = 12,
