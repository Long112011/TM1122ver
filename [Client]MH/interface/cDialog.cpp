...
cImage ToolTipImageGG;
cImage ToolTipImageGGj;
void cDialog::Init(LONG x, LONG y, WORD wid, WORD hei, cImage * basicImage, LONG ID)
{
	cWindow::Init(x, y, wid, hei, basicImage, ID);

	SCRIPTMGR->GetImage(63, &ToolTipImageGG, PFT_HARDPATH);
	SCRIPTMGR->GetImage(1, &ToolTipImageGGj, PFT_HARDPATH);
}
...
void cDialog::RenderJack()
{
	if (PtInWindow(MOUSE->GetMouseEventX(), MOUSE->GetMouseEventY()))
	{
		VECTOR3 Posisi = { 0, 0, 0 };

		char temp[64] = { 0, };

		Posisi.x = (GetAbsX() - MOUSE->GetMouseX() / .01f / 100);
		Posisi.y = (GetAbsY() - MOUSE->GetMouseY() / .01f / 100);
		Posisi.z = ((GetAbsY() - GetAbsY()) - MOUSE->GetMouseY() / .01f / 100);

		sprintf(temp, "x%1.0f\t,\ty%1.0f", Posisi.x, Posisi.y);
		this->SetToolTip(RemoveDash(temp), RGBA_MAKE(255, 255, 255, 255), &ToolTipImageGG, TTCLR_DEFAULT);
		...
		{
			sprintf(temp, "x: %1.0f , y: %1.0f", SavePos.x, SavePos.y);
			AddToolTipLine(RemoveDash(temp), RGBA_MAKE(255, 0, 0, 255), &ToolTipImageGGj);

			sprintf(temp, "width: %1.0f , height: %1.0f", SavePos.x - Posisi.x, SavePos.z - Posisi.z);
			AddToolTipLine(RemoveDash(temp), RGBA_MAKE(0, 255, 0, 255));

			if (MOUSE->RButtonUp())
			{
				sprintf(temp, "%1.0f %1.0f %1.0f %1.0f",
					SavePos.x,
					SavePos.y,
					SavePos.x - Posisi.x,
					SavePos.y - Posisi.y
					);
				CHATMGR->AddMsg(CTC_SYSMSG, RemoveDash(temp));
			}
		}
	}
}
