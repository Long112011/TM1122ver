...
void cIcon::Init(LONG x, LONG y, WORD wid, WORD hei, cImage * basicImage, LONG ID)
{
	...
	SCRIPTMGR->GetImage(65, &m_LockHoldImage);
...
void cIcon::Render()
{
	...
	if( m_bLock )
		m_LockImage.RenderSprite( NULL, NULL, 0, &m_absPos, RGBA_MERGE(0xffffff, m_alpha * m_dwOptionAlpha / 100));

	if (m_bLockHold)
		m_LockHoldImage.RenderSprite(NULL, NULL, 0, &m_absPos, RGBA_MERGE(0x0000ff, m_alpha * m_dwOptionAlpha / 100));
...
void cIcon::SetLockHold(BOOL bLockHold)
{
	m_bLockHold = bLockHold;
}
