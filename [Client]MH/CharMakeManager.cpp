...
//			if( HELPERMGR->IsCompleteInit() )
			...
			if (m_pNewPlayer)
			{
				...
				{
				case WP_AXE:
					m_pNewPlayer->ChangeMotion(eMotion_Battle_Chang_Standard, 1); // ¸«Í·¶¯×÷±àºÅ£¨Äã¶¨ÒåºÃµÄ£©
					m_pNewPlayer->ChangeBaseMotion(eMotion_Battle_Chang_Standard);
					break;
				case WP_DAGGER:
					m_pNewPlayer->ChangeMotion(eMotion_Battle_Amgi_Standard, 1); // ´Ì¿Í¶¯×÷±àºÅ£¨Äã¶¨ÒåºÃµÄ£©
