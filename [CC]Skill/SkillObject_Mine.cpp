...
DWORD CSkillObject_Mine::Update()
{
	...
	if( rt == SO_DESTROYOBJECT )
	{
		...
		OBJECTMGR->GetTargetInRange(&pos,GetSkillInfo()->GetSkillRange(),&objarray,FALSE,0,0);
...
#else
		ObjectArray<LPOBJECT> objarray;
		OBJECTMGR->GetTargetInRange(&pos2,GetSkillInfo()->GetSkillRange(),&objarray,FALSE,0,0);
