	...
	if(pList->BuffList[0])
	{
		...
		for( int i = 0; i < MAX_PET_BUFF_NUM; ++i )
		{
			...
			if(pBuffList && pBuffList->BuffValueData)
			{
				...
				else if (pBuffList->BuffKind == ePB_MaxLifeNaeRyukShield)////最大生命，内力，护体 增加 % 技能特技+角色自身的百分百 10
				{
					wsprintf(BuffName, CHATMGR->GetChatMsg(1477));
				}
				else if (pBuffList->BuffKind == ePB_Exp)//经验增加%
				{
					wsprintf(BuffName, CHATMGR->GetChatMsg(1477));
				}
				else if (pBuffList->BuffKind == ePB_MoneyDropUp)//金钱掉落率增加%
				{
					wsprintf(BuffName, CHATMGR->GetChatMsg(1477));
				}
				else if (pBuffList->BuffKind == ePB_PVPDmgSub)///PVP受到伤害减少 13
				{
					wsprintf(BuffName, CHATMGR->GetChatMsg(1477));
				}
				else if (pBuffList->BuffKind == ePB_PVPDmgUp)//  //PVP伤害增加 14
				{
					wsprintf(BuffName, CHATMGR->GetChatMsg(1477));
				}
				//--------------------------
...
void CItemManager::RemoveUsedItemInfo(WORD wIndex, DWORD dwDBIdx)
{
	...
	{
		if (PETMGR->IsSummonPet() == FALSE)//刀哥  3pet
