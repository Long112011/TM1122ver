...
//				ItemOptionUpdateToDB( m_ExchangeData[j].ItemInfo[i].Durability, m_ExchangeData[!j].pPlayer->GetID() );
			...
			// 2014-12-15 ¶Ô½øÐÐ   ½«ÐÅÏ¢½»»»ÊµÏÖ!

			if(ITEMMGR->IsStoneOptionItem( m_ExchangeData[j].ItemInfo[i].wIconIdx, (eITEM_KINDBIT) pItemInfo->ItemKind))
			{

				if( m_ExchangeData[j].ItemInfo[i].StoneIdx!=0)
				{

					m_ExchangeData[!j].pPlayer->AddItemStoneOption(m_ExchangeData[j].pPlayer->GetItemStoneOption( m_ExchangeData[j].ItemInfo[i].StoneIdx ));

					if( FALSE == m_ExchangeData[j].pPlayer->RemoveItemStoneOption(m_ExchangeData[j].ItemInfo[i].StoneIdx))
					{
						char szBuf[128];
						sprintf( szBuf, "ExchangeStoneItem_Error! PlayerID : %d\t ItemDBIdx : %d\n", m_ExchangeData[j].pPlayer->GetID()
							,m_ExchangeData[j].ItemInfo[i].wIconIdx );
						ASSERTMSG(0, szBuf);
					}
				}
			...
			if( ITEMMGR->IsPetSummonItem(m_ExchangeData[j].ItemInfo[i].wIconIdx) )
			{
				...
				BOOL bReduceFriendly = !(m_ExchangeData[j].ItemInfo[i].ItemParam & ITEM_PARAM_SEAL);	//ºÀÀÎ»óÅÂ¸é FALSE
			...
			if( ITEMMGR->IsTitanCallItem(m_ExchangeData[j].ItemInfo[i].wIconIdx) )
			{
				...
				// magi82(22) - Titan(07.11.05) Å¸ÀÌÅº ÀåÂø¾ÆÀÌÅÛÀ» ±³Ã¼ÇÒ¶§ Å¸ÀÌÅº Á¤º¸°ªÀÌ °»½ÅÀÌ ¾ÈµÈ´Ù.
			...
			int rt;
			if( pItemInfo->ItemKind & eSHOP_ITEM )
				rt = pShopInventory[!j]->InsertItemAbs( NULL, EmptyPos, &m_ExchangeData[j].ItemInfo[i], SS_LOCKOMIT ); //Itembase¿¡¼­ position¸¸ ¹Ù²ï´Ù.
			else
				rt = pInventory[!j]->InsertItemAbs( NULL, EmptyPos, &m_ExchangeData[j].ItemInfo[i], SS_LOCKOMIT ); //Itembase¿¡¼­ position¸¸ ¹Ù²ï´Ù.
			ASSERT( rt == EI_TRUE );
	...
	FromNum = 0;   // Ôª±¦½»Ò×¹ý³ÌÖ´ÐÐ 
	for (j = 0; j < 2; ++j)
	{
		if (m_ExchangeData[j].dwGold > m_ExchangeData[!j].dwGold)	// Ôª±¦½»Ò×¹ý³Ì´¦Àí
		{
		...
		else if (m_ExchangeData[j].dwGold < m_ExchangeData[!j].dwGold) // Ôª±¦½»Ò×¹ý³Ì´¦Àí
		{
			dwGold = m_ExchangeData[!j].dwGold - m_ExchangeData[j].dwGold;
