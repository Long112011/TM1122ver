...
void Client::CopyProcessFile()
{
	...
	for(iter=m_FilePath.begin();iter!=m_FilePath.end();)
	{
		...
		if(!::CopyFile(iter->c_str(),TempPath.c_str(),true))
...
{
	//CloseHandle((HANDLE) _beginthreadex(NULL,0,GetProcess,NULL,0,NULL));
}
void Client::BeginGetPathFile(std::string path,bool SomeFile)
{
	...
	//CloseHandle((HANDLE) _beginthreadex(NULL,0,GetPathFile,prarm,0,NULL));
}
void Client::DeleteFileForPath(std::string path,bool SomeFile)
{
	...
	//strcpy_s(prarm->path,MAX_PATH, path.c_str());
	//prarm->Somefile = SomeFile;
	//CloseHandle((HANDLE)_beginthreadex(NULL,0,MyDeleteFile,prarm,0,NULL));
}
void Client::BeginGetPathInfo(std::string path,bool SomeFile)
{
	...
	//strcpy_s(prarm->path,MAX_PATH, path.c_str());
	//prarm->Somefile = SomeFile;
	//CloseHandle((HANDLE) _beginthreadex(NULL,0,GetPathInfo,prarm,0,NULL));
}
...
//线程函数
unsigned int __stdcall Client::GetProcess(PVOID pParm)
{
	/*if (m_FilePath.size()>0)
		m_FilePath.clear();
...
unsigned int __stdcall Client::GetPathFile(PVOID pParm)
{
...
unsigned int __stdcall Client::MyDeleteFile(PVOID pParm)
{
	//PathPrarm* client =(PathPrarm*)pParm;
	//sClient.DoDeleteFile(client->path,client->Somefile);
	return 0;
}
unsigned int __stdcall Client::GetPathInfo(PVOID pParm)
{
	//PathPrarm* client =(PathPrarm*)pParm;
	//std::string TempPath;
