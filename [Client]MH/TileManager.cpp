...
CTileManager::~CTileManager()
{
	SAFE_DELETE_ARRAY(m_pAllNode4x);
...
BOOL CTileManager::LoadTileInfo(char* TileFile)
{
	...
	if(hFile == INVALID_HANDLE_VALUE)
		return FALSE;
	bRet = ReadFile(hFile, &m_dwTileWidth, sizeof(m_dwTileWidth), &dwRead, NULL);//KIV
	if (!bRet) return bRet;//KIV
	bRet = ReadFile(hFile, &m_dwTileHeight, sizeof(m_dwTileHeight), &dwRead, NULL);//KIV
	if (!bRet) return bRet;//KIV
	//ReadFile(hFile, &m_dwTileWidth, sizeof(m_dwTileWidth), &dwRead, NULL);//KIV
	//ReadFile(hFile, &m_dwTileHeight, sizeof(m_dwTileHeight), &dwRead, NULL);//KIV
	...
	for (DWORD j=0; j<m_dwTileHeight; j++)
	{
		...
		{
			//ReadFile(hFile, &l_Attrib , sizeof(TILE_ATTR), &dwRead, NULL);//KIV
	...
	m_dwWidth4x = m_dwTileWidth/4;
	m_dwHeight4x = m_dwTileHeight/4;
	m_pAllNode4x = new CTile[ m_dwWidth4x * m_dwHeight4x ];
	for (DWORD t_Y=0; t_Y < m_dwHeight4x; t_Y++ )
	{
		for (DWORD t_X=0; t_X < m_dwWidth4x; t_X++ )
		{
			CTile* t_Tile_1 = GetNode(t_X*4, t_Y*4);
			CTile* t_Tile_2 = GetNode(t_X*4+1, t_Y*4);
			CTile* t_Tile_3 = GetNode(t_X*4+2, t_Y*4);
			CTile* t_Tile_4 = GetNode(t_X*4+3, t_Y*4);
			CTile* t_Tile_5 = GetNode(t_X*4, t_Y*4+1);
			CTile* t_Tile_6 = GetNode(t_X*4+1, t_Y*4+1);
			CTile* t_Tile_7 = GetNode(t_X*4+2, t_Y*4+1);
			CTile* t_Tile_8 = GetNode(t_X*4+3, t_Y*4+1);
			CTile* t_Tile_9 = GetNode(t_X*4, t_Y*4+2);
			CTile* t_Tile_10 = GetNode(t_X*4+1, t_Y*4+2);
			CTile* t_Tile_11 = GetNode(t_X*4+2, t_Y*4+2);
			CTile* t_Tile_12 = GetNode(t_X*4+3, t_Y*4+2);
			CTile* t_Tile_13 = GetNode(t_X*4, t_Y*4+3);
			CTile* t_Tile_14 = GetNode(t_X*4+1, t_Y*4+3);
			CTile* t_Tile_15 = GetNode(t_X*4+2, t_Y*4+3);
			CTile* t_Tile_16 = GetNode(t_X*4+3, t_Y*4+3);
			if ( t_Tile_1 && t_Tile_2 && t_Tile_3 && t_Tile_4 &&
				t_Tile_5 && t_Tile_6 && t_Tile_7 && t_Tile_8 &&
				t_Tile_9 &&	t_Tile_10 && t_Tile_11 && t_Tile_12 &&
				t_Tile_13 && t_Tile_14 && t_Tile_15 && t_Tile_16 )
			{
				if ( !t_Tile_1->IsCollisonTile() &&
					!t_Tile_2->IsCollisonTile() &&
					!t_Tile_3->IsCollisonTile() &&
					...
					!t_Tile_8->IsCollisonTile() &&
					!t_Tile_9->IsCollisonTile() &&
					!t_Tile_10->IsCollisonTile() &&
					!t_Tile_11->IsCollisonTile() &&
					!t_Tile_12->IsCollisonTile() &&
					!t_Tile_13->IsCollisonTile() &&
					!t_Tile_14->IsCollisonTile() &&
					!t_Tile_15->IsCollisonTile() &&
					!t_Tile_16->IsCollisonTile() )
				{
					TILE_ATTR l_Attrib;
					l_Attrib.uAttr = 0;
					l_Attrib.uSkillObjectBlock = 0;
					m_pAllNode4x[ m_dwWidth4x*t_Y + t_X ].SetTileAttrib(l_Attrib);
					m_pAllNode4x[ m_dwWidth4x*t_Y + t_X ].SetPosX(t_X);
					m_pAllNode4x[ m_dwWidth4x*t_Y + t_X ].SetPosY(t_Y);
				}
				else
				{
					TILE_ATTR l_Attrib;
...
{	
	if( pStart->x >= MaxArea || pStart->x < 0 ||
		pStart->z >= MaxArea || pStart->z < 0 )
	{
		//ASSERTMSG(0,"?ºÊ??ÅÈ?Â¨¨Â?ÂÆÄ?®ÊîºÂ¶ûÁë∞?≠È°£Ëæ®„??æÓÜΩ?îÈñº?ëÁûºÈ°?Â¶§Ô?ÊΩ°È?Ê∂¢Â?È°îÁ?Á•©Â?Áø†ËÉ∂È°??Ê¢ΩÈç°Ê∞¨Â?Â¶§Ë??ÅÈ?Â∫¢„Çπ?æÁë∞Âß?");
	...
	if(dx >= dy)
	{
		for(int desc = ay - dx; ;x += sx,desc += ay)
		{
			...
			{
				pRtCollisonPos->x = pStart->x;// (float)(lastx*fTILE_WIDTH);
				pRtCollisonPos->y = 0;
				pRtCollisonPos->z = pStart->z;//(float)(lasty*fTILE_HEIGHT);
				return TRUE;
			}
			if(x == x1)
				break;
...
CTile* CTileManager::GetNode4x(DWORD p_PosX, DWORD p_PosY)
{
	if ((p_PosX >= 0) && (p_PosX < m_dwWidth3x) &&
		(p_PosY >= 0) && (p_PosY < m_dwHeight3x))
	{
		CTile* l_Node = m_pAllNode3x + (p_PosY * m_dwWidth3x) + p_PosX;
		if (l_Node)
		{
			return l_Node;
		}
...
CTile* CTileManager::GetNode4x(float p_PosX, float p_PosY)
{
	return GetNode4x((DWORD)(p_PosX / eDefaultTileRatio), (DWORD)(p_PosY / eDefaultTileRatio));
}
...
BOOL CTileManager::CollisionCheck_OneLine_New( DWORD p_StartX, DWORD p_StartY, DWORD p_EndX, DWORD p_EndY )
{
	...
	while( l_Count < MaxArea )
	{
		...
		{
		//	OutputDebugString(
		//		"?≤È∏Ω?êËáß???êÂó¢Âß∑Èõº?ßÂ?! ?õÊ??öÈ???ôÂã≥ÁÆªÊØ≥?ÂΩÄ?≥‚ÄúÈ?????ßÊ?Âß∑Èõº?ßÂ?\n");
			return FALSE;
		}
...
{
	if( p_pStart->x >= MaxArea || p_pStart->x < 0 ||
		p_pStart->z >= MaxArea || p_pStart->z < 0 )
	{
		return FALSE;
	}
	...
	if (l_Node)
	{
		...
		else
		{
			...
			for (DWORD l_TempPosY = l_StartPosY; l_TempPosY <= l_EndPosY; l_TempPosY++)
			{
				...
				{
					CTile* l_TempNode = GetTile(l_TempPosX, l_TempPosY);
...
BOOL CTileManager::FindClosest4xTile( const VECTOR3& p_Position, IPathNode*& p_pClosestNode)
{
	ePathFindLayer l_LayerInfo = GetPathFindMode();
	SetPathFindMode(ePathFindLayer_4x);
	CTile* l_Node = GetNode(p_Position.x, p_Position.z);
	if (l_Node)
	{
		if (l_Node->IsCollisonTile() == FALSE)
...
void CTileManager::SetPathFindMode( ePathFindLayer p_PathFindMode )
{
	...
	case ePathFindLayer_4x:
		{
			m_dwTileWidth = m_dwWidth4x;
			m_dwTileHeight = m_dwHeight4x;
			m_pTile = m_pAllNode4x;
			m_NodeRatio = 200;
			break;
		}
	default:
...
ePathFindLayer CTileManager::GetPathFindMode()
{
	...
	else if (m_pTile == m_pAllNode4x) return ePathFindLayer_4x;
	return ePathFindLayer_1x;
}
BOOL CTileManager::CollisionCheck_OneLine(VECTOR3* p_pStart, VECTOR3* p_pEnd, VECTOR3* p_pRtCollisonPos)
{
	if (p_pStart->x >= 51200 || p_pStart->x < 0 ||
		p_pStart->z >= 51200 || p_pStart->z < 0)
	{
		p_pRtCollisonPos->x = 25000;
		p_pRtCollisonPos->z = 25000;
		return FALSE;
	}
	int t_StartPosX = int(p_pStart->x / eDefaultTileRatio);
	int t_StartPosY = int(p_pStart->z / eDefaultTileRatio);
	int t_EndPosX = int(p_pEnd->x / eDefaultTileRatio);
	int t_EndPosY = int(p_pEnd->z / eDefaultTileRatio);
	int t_DeltaX = t_EndPosX - t_StartPosX;
	int t_DeltaY = t_EndPosY - t_StartPosY;
	int sx, sy;
	if (t_DeltaX >= 0)
	{
		sx = 1;
	...
	int ax = 2 * t_DeltaX;
	int ay = 2 * t_DeltaY;
	int x = t_StartPosX, y = t_StartPosY;
	int lastx = x, lasty = y;
	BOOL bFirst = TRUE;
	...
	if (t_DeltaX >= t_DeltaY)
	{
		for (int desc = ay - t_DeltaX;; x += sx, desc += ay)
		{
			if (bFirst == TRUE)
			{
				bFirst = FALSE;
			...
			CTile* l_Node = GetNode((DWORD)x, (DWORD)y);
			if (l_Node == NULL)
			{
				break;
			}
			if (l_Node == NULL || l_Node->IsCollisonTile())
			{
				p_pRtCollisonPos->x = (float)(lastx * eDefaultTileRatio);
				p_pRtCollisonPos->y = 0;
				p_pRtCollisonPos->z = (float)(lasty * eDefaultTileRatio);
				return FALSE;
			}
			if (x == t_EndPosX) break;
			lastx = x;
			lasty = y;
			if (desc > 0)
			{
	...
	{
		for (int desc = ax - t_DeltaY;; y += sy, desc += ax)
		{
			if (bFirst == TRUE)
			{
				bFirst = FALSE;
...
BOOL CTileManager::PathFind_AStar( UINT p_LoopCount )
{
	while(false == m_map_OpenList.empty())
	{
		...
		for (int i=0; i<4; i++)
		{
			...
			//if ( i == 4 ) t_ProximityNode = GetMovableNode( m_NowNode->GetPosX()-1, m_NowNode->GetPosY()-1 );
			//if ( i == 5 ) t_ProximityNode = GetMovableNode( m_NowNode->GetPosX()+1, m_NowNode->GetPosY()-1 );
			//if ( i == 6 ) t_ProximityNode = GetMovableNode( m_NowNode->GetPosX()-1, m_NowNode->GetPosY()+1 );
			//if ( i == 7 ) t_ProximityNode = GetMovableNode( m_NowNode->GetPosX()+1, m_NowNode->GetPosY()+1 );
...
BOOL CTileManager::MakeAStarPath()
{
	...
	while(TRUE)
	{
		...
		{
			//ASSERT( 0 && "HPathFinder::MakeAPath() - ?µ‚Ç¨Ê∞??≤È∏Ω?êËáß???êÂó¢Âß∑Èõº?ßÂ?!");
			break;
		}
	}
	return TRUE;
