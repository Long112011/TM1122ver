...
//	STARTLOG(GAMERESRCMNGR->LoadTitanEquipEnduranceInfo());
	...
	ITEMMGR->LoadSetItemOption();
		ITEMMGR->LoadSetItemQualityOption();//¼ÓÔØ×°±¸Æ·ÖÊÐÅÏ¢
...
// RaMa -04.11.24
	while( !file.IsEOF() )
	{
		...
		else if (strcmp(Token, "#MUGONGPHY") == 0)//PVPÎïÀí¹¥»÷
		{
			gEventRateFile[eEvent_MugongPhy] = file.GetFloat();
		}
		else if (strcmp(Token, "#MUGONGATTR") == 0)//PVPÊôÐÔ¹¥»÷
		{
			gEventRateFile[eEvent_MugongAttr] = file.GetFloat();
		}
		else if (strcmp(Token, "#MUGONGKILLMR") == 0)//PVEÉËº¦±¶ÂÊ Õ½Ê¿·¨Ê¦¹²ÓÃ
		{
			gEventRateFile[eEvent_MugongKillMonster] = file.GetFloat();
		}
		else if (strcmp(Token, "#MUGONGPHYCRI") == 0)//PVP/PVE±©»÷±¶ÂÊ  Õ½Ê¿
		{
			gEventRateFile[eEvent_MugongPhyCritical] = file.GetFloat();
		}
		else if (strcmp(Token, "#MUGONGATTCRI") == 0)//PVP/PVE±©»÷±¶ÂÊ  ·¨Ê¦
		{
			gEventRateFile[eEvent_MugongAttrCritical] = file.GetFloat();
		}
		else if (strcmp(Token, "#AssassinPhyDmg") == 0)//´Ì¿ÍµÄÎïÀíÉËº¦Ìí¼ÓÂÊ
		{
