...
CItemShopInven::CItemShopInven()
{
	...
	m_nIconType = WT_ITEM;	//�?그리드다이얼로그에서 다룰 아이�?종류
}
...
BOOL CItemShopInven::InPt(POSTYPE absPos)
{
	return (TP_SHOPINVEN_START<=absPos&&absPos<TP_SHOPINVEN_END-TABCELL_SHOPINVEN_NUM);
}
...
{
	return ( absPos - TP_SHOPINVEN_START ) % (40+TABCELL_SHOPINVEN_NUM)/*80*/;
}
...
{
	if(InPt(absPos)/*TP_SHOPINVEN_START <= absPos && absPos < TP_SHOPINVEN_END-TABCELL_SHOPINVEN_PLUS_NUM*/)
	{
...
BOOL CItemShopInven::CanBeMoved(CItem* pItem,POSTYPE pos)
{
	...
	if( TP_SHOPITEM_START <= pos && pos < TP_SHOPINVEN_END/*-TABCELL_SHOPINVEN_PLUS_NUM*/ )
	{
		POSTYPE fromPos = pItem->GetPosition();
		if( TP_SHOPITEM_START <= fromPos && fromPos < TP_SHOPINVEN_END/*-TABCELL_SHOPINVEN_PLUS_NUM*/ )
			return TRUE;
		else
			return FALSE;
