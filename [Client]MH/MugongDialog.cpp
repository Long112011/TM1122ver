...
void CMugongDialog::RefreshSlot()
{
	if (!HERO)			return;
	if (HERO->GetExtraMugongSlot() <= 3)
	{
		exM = HERO->GetExtraMugongSlot();
		exM2 = 0;
	}
	if (HERO->GetExtraMugongSlot()>3)
	{
		exM = 3;
		exM2 = (HERO->GetExtraMugongSlot() - exM);
	}
	m_pMugongRect[MUGONGWINDOW_NORMAL].SetDiableIcon((POSTYPE)(GIVEN_MUGONG_SLOT + (MUGONG_SLOT_ADDCOUNT * exM)), (POSTYPE)25);
	m_pMugongRect[MUGONGWINDOW_EXTEND].SetDiableIcon((POSTYPE)(MUGONG_SLOT_ADDCOUNT * exM2), (POSTYPE)25);
}
void CMugongDialog::SetActive(BOOL val)
{
	...
	if (val)
	{
		...
		{
			GAMEIN->GetMugongDialog()->SetMugongExtend(FALSE);
		}
...
BOOL CMugongDialog::GetBlankPositionRef(BYTE type, WORD & absPos)
{
	...
	case eMUGONGBOOK:
	{
						...
						{
							DWORD Slot = exM;
							DWORD cp = absPos - TP_MUGONG1_START;
						...
						{
							DWORD Slot = exM2;
							DWORD cp = absPos - TP_MUGONG2_START;
							if (cp >= (MUGONG_SLOT_ADDCOUNT*Slot))
								return FALSE;
							else
...
void CMugongDialog::AddMugong(POSTYPE pos, CMugongBase * mugong)
{
	ASSERT(mugong);
...
MLOC CMugongDialog::GetWindowLocate(POSTYPE abs_pos)
{
...
BOOL CMugongDialog::DeleteMugong(POSTYPE pos, CMugongBase ** mugong)
{
	ASSERT(mugong);
	MLOC loc = GetWindowLocate(pos);
	switch (loc)
...
void CMugongDialog::MoveMugong(POSTYPE ToPos, POSTYPE FromPos)
{
...
BOOL CMugongDialog::CanBeMoved(cIcon * pIcon, POSTYPE abs_pos)
{
	...
	{
								DWORD Slot = exM;
								DWORD cp = abs_pos - TP_MUGONG1_START;
								if (cp >= GIVEN_MUGONG_SLOT + (MUGONG_SLOT_ADDCOUNT*Slot))
									return FALSE;
								else
	...
	{
								DWORD Slot = exM2;
								DWORD cp = abs_pos - TP_MUGONG2_START;
								if (cp >= (MUGONG_SLOT_ADDCOUNT*Slot))
									return FALSE;
								else
									return m_pMugongRect[loc].CanBeMoved(pIcon, abs_pos);
...
void CMugongDialog::ExcuteMugong(POSTYPE pos)
{
	...
	case MUGONGWINDOW_JINBUB:
	{
								...
								else if (pInfo->GetSkillInfo()->TargetKind == 1)
								{
...
void CMugongDialog::Linking()
{
	...
	m_pMugongRectBtn[MUGONGWINDOW_TITAN] = (cPushupButton*)GetWindowForID(MGI_MUGONG_TITAN_PUSHUP);
...
void CMugongDialog::SetSkillInfoStatic(CMugongBase* pMugongBase)
{
	...
	{
		m_pMugongInfo[MGI_EXPPOINT]->SetStaticText(CHATMGR->GetChatMsg(480));
	}
}
