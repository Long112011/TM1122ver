...
public:
	...
	BOOL Init(char * txt, int MaxButton, MENU_CUSTOM_INFO* pMenuInfo, void(*CommandFunc)(char* szCommand));
	void Release();
	...
	void LOC(DWORD color,int LogLevel,char* strMsg,...);

	void LOGANDFILE(char* FilePath, DWORD color, char* strMsg, ...);
	//void LOGEX(int LogLevel,DWORD color,char* strMsg,...);
