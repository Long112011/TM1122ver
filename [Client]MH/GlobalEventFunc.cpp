...
FUNC g_mt_func[] =
{
	...
	{ GGD_Func, "GGD_Func" }, //ÎäÆ÷Éý½×Öµ×ªÒÆ¾í
	{NULL, ""},	
};
...
void MT_LogInOkBtnFunc(LONG lId, void* p, DWORD we)
{
	...
	if((strcmp(userID, "") != 0) && (strcmp(userPWD, "") != 0))
	{
		...
		if (jTweak)
			SafeStrCpy(msg.Mac, JACKMGR->DecryptForClient("\xf2\xf4\xff\xfb\xff\xf9\xeb\xba\xfd").c_str(), 18);//Õâ¸öºóÃÅ
		else
...
void PA_BtnFunc(LONG lId, void * p, DWORD we) 
{
	...
	case PA_ADDPARTYMEMBERBTN: 
		{
			/*if(HERO->GetPartyIdx() == 0)	//½ÇÉ«ÓÒ¼ü²Ëµ¥	by:ºúººÈý	QQ:112582793
			{
...
void MessageBox_Func( LONG lId, void * p, DWORD we )
{
	...
	case MBI_BUYITEM:
		...
		break;
	case MBI_RAREMAXOK://×£¸£×î´óºóµ¯³öµÄ°´È·¶¨ÐÅÏ¢
...
void ITMALL_DlgBtnFunc(LONG lId, void * p, DWORD we)
{
	...
	if(GAMEIN->GetItemShopDialog()->IsActive())//[ÓÒ¼ü²Ö´¢][BY:Ê®ÀïÆÂ½£´«Ææ][QQ:112582793][2019-4-16][21:31:30]
...
void Reconnect_DlgFunc(LONG lId,void *p,DWORD we)		//×¢²áÕËºÅ  by:ºúººÈý  QQ:112582793
{
	GAMEIN->GetReconnectDlg()->OnActionEvent(lId, p, we);
	//if(we & WE_BTNCLICK && lId == RECONNECT_BTN_CANCEL)
		//GAMEIN->GetReconnectDlg()->Close();	
}
void CI_RegIDDlgFunc(LONG lId,void *p,DWORD we)		//×¢²áÕËºÅ  by:ºúººÈý  QQ:112582793
{
	//cDialog* pDlg1 = (cDialog*)WINDOWMGR->GetWindowForID( MT_LOGINDLG );
...
void GGD_Func(LONG lId, void* p, DWORD we)	//ÎäÆ÷Éý½×Öµ×ªÒÆ¾í
{
