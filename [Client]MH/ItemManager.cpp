								  ...
								  BOOL IsGoldShop)
{
	...
	if(bits & eEQUIP_ITEM)
	{
		if(bNormalTip==0)
		{
			...
			pIcon->AddToolTipLine( JACKJACK );
			SetEquipItemToolTip(pIcon,pInfo,pOptionInfo,pRareOptionInfo,pStoneOptionInfo,bIsQuey);
			pIcon->AddToolTipLine( JACKJACK );
		}
...
void CItemManager::AddGoldShopToolTip( cIcon* pIcon, ITEM_INFO* pItemInfo )
{
	...
	//[·Ö¸îÏß][By:Ê®ÀïÆÂ½£Éñ][QQ:112582793][2017/11/28]
//	pIcon->AddImagTopLine(&imgTopLine,pIcon->GetTipLineCount());
...
void CItemManager::SetEquipItemToolTip( cIcon* pIcon, ITEM_INFO* pInfo, ITEM_OPTION_INFO * pOptionInfo, ITEM_RARE_OPTION_INFO* pRareOptionInfo,ITEM_STONE_OPTION_INFO * pStoneOptionInfo,BOOL bIsQuey)
{
	CExchangeItem* pExcItem = NULL;
	CItem * pcItem = NULL;
	if( pIcon->GetType() == WT_EXCHANGEITEM || pIcon->GetType() == WT_STALLITEM || pInfo->ItemKind == eSHOP_ITEM_PET ||pIcon->GetType() == WT_BUYITE)
...
void CItemManager::SetPetSummonItemToolTip(cIcon * pIcon, ITEM_INFO * pInfo, DWORD dwItemDBIdx, BOOL IsGoldShop)
{
	...
	if (IsGoldShop && pInfo->ItemIdx == 55889)//Éñùm
	{
			...
			case eItemTable_Weared:
				{
					...
					if(bits & eYOUNGYAK_ITEM)
					{
						if(item->GetDurability() > 1)
						{
							...
							if (Tmp)
							{
								HERO->GetDelayGroup()->AddYoungYakPotionDelay(Tmp->DelayTime, Tmp->wtype);   //[ÉèÖÃÒ©Æ·»ØÆøÊ±¼ä][2017/11/27]
							}
						}
						else
						{
