...
void CQuickDialog::MyView()
{
	COptionDialog * m_Dlg = GAMEIN->GetOptionDialog();
	if (((cCheckBox*)((cDialog*)m_Dlg->GetTabSheet(2))->GetWindowForID(OTI_CB_GAMMA))->IsChecked() ||
		OPTIONMGR->GetGameOption()->bGamma)
		MyCameraEngine->ResetCamera(MAP->GetMapDefaultSight(), DEGTORAD(GAMEIN->GetOptionDialog()->GetFovVal()));
	else
		MyCameraEngine->ResetCamera(MAP->GetMapDefaultSight(), 1000.f);
}
...
void CQuickDialog::Render()
{
	...
	if (isConnected)
	{
		...
		for (int i = 0; i < TABCELL_QUICK_NUM; ++i)
		{			
			if (i != 0 + column & i != 1 + column & i != 2 + column)
			{
				cImage	m_JackIcon;
				SCRIPTMGR->GetImage(137, &m_JackIcon, PFT_HARDPATH);
				m_JackIcon.RenderSprite(NULL, NULL, 0, &Pos, RGBA_MERGE(m_dwImageRGB, 180));
			}
		...
		for (int i = 0; i < TABCELL_QUICK_NUM; ++i)
		{
			...
			{
				cImage	m_JackIcon;
				SCRIPTMGR->GetImage(143, &m_JackIcon, PFT_JACKPATH);
				m_JackIcon.RenderSprite(NULL, NULL, 0, &Pos, RGBA_MERGE(m_dwImageRGB, m_alpha * m_dwOptionAlpha / 100));
			}
			if ((1 + column) == i)
			{
				cImage	m_JackIcon;
				SCRIPTMGR->GetImage(144, &m_JackIcon, PFT_JACKPATH);
				m_JackIcon.RenderSprite(NULL, NULL, 0, &Pos, RGBA_MERGE(m_dwImageRGB, m_alpha * m_dwOptionAlpha / 100));
			}
			if ((2 + column) == i)
			{
				cImage	m_JackIcon;
				SCRIPTMGR->GetImage(145, &m_JackIcon, PFT_JACKPATH);
				m_JackIcon.RenderSprite(NULL, NULL, 0, &Pos, RGBA_MERGE(m_dwImageRGB, m_alpha * m_dwOptionAlpha / 100));
			}
...
void CQuickDialog::FakeAddItem(LONG x, LONG y, CItem * pItem)
{
	...
	if (pItem->GetItemIdx() == eSundries_RareItemCreate99)
	{
		return;
	}
...
void CQuickDialog::SetToolTipIcon(CQuickItem * item)
{
	...
	if (pInfo)
	{
		...
		if (pInfo->ItemKind == eYOUNGYAK_ITEM)
		{
			...
			{
				wsprintf(ToolTipText, "%s %d%%", "护体恢复", 100);
				item->AddToolTipLine(ToolTipText, TTTC_EXTRAATTR);
			}
