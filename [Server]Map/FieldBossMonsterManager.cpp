			...
			if( pGroup->m_pBoss )
			{
				...
				if( !pGroup->m_pBoss->IsBattle() )
				{
					if( pGroup->m_pBoss->IsDistruct() )
					{
						...
						//fprintf(file, "%s\t%s\t%d\t%u\t%d\t%u\t%f\t%f\t%u\n",
						//	"Destruction of the BOSS", GetCurTimeToString(),pGroup->m_pBoss->GetMonsterKind(), pGroup->m_pBoss->GetID(), pGroup->m_pBoss->GetGridID(), gCurTime,
...
void CFieldBossMonsterManager::BossDead(CFieldBossMonster* pBoss)
{
	...
	if(pGroup)
	{
		if( pGroup->m_pBoss == pBoss )
		{
			...
			//FILE* file = fopen(m_LogFile, "a+");
			//fprintf(file, "%s\t%s\t%d\t%u\t%d\t%u\t%f\t%f\t%u\n",
			//	"Death of the BOSS", GetCurTimeToString(),pBoss->GetMonsterKind(), pBoss->GetID(), pBoss->GetGridID(), gCurTime,
			//	Pos.x, Pos.z, 0);
			//fclose(file);
...
void CFieldBossMonsterManager::AddFieldBossMonster( CFieldBossMonster* pBoss )
{
	...
	//FILE* file = fopen(m_LogFile, "a+");
	//fprintf(file, "%s\t%s\t%d\t%u\t%d\t%u\t%f\t%f\t%u\n",
	//	"Field BOSS Spawned",GetCurTimeToString(), pBoss->GetMonsterKind(), pBoss->GetID(), pBoss->GetGridID(), gCurTime,
	//	Pos.x, Pos.z, 0);
	//fclose(file);
	...
	if( !pGroup )
	{
		...
		fprintf(file, "%s\t%d\t%u\t%d\t%u\t\n",
			"Packet memory allocation not received", pBoss->GetMonsterKind(), pBoss->GetID(), pBoss->GetGridID(), gCurTime);
		fclose(file);
		return;
	}
