...
struct USERINFO
{
	...
	char	strPhon[20];				
	char	strEmail[50];				
	char	strRegDateTime[30];			
	char	strLastLoginTime[30];		
	char	strLastLogoutTime[30];		
	char	strTotalTime[50];			
	int		nTotalTime;					
	WORD	wUserLevel;					
};
...
struct TMSG_CHARACTERLOG : public TMSGBASE	
{
	BOOL		bEnd;
	WORD		wCount;
	DWORD		dwCharIdx;
...
struct TMSG_EXPPOINTLOG : public TMSGBASE	
{
	BOOL		bEnd;
	WORD		wCount;
	DWORD		dwCharIdx;
...
struct DELCHARLIST	
{
	DWORD	dwcharacter_idx;			
	DWORD	dwuser_idx;					
	char	character_name[17];			
	WORD	wflag;						
	char	create_date[11];			
	char	create_datetime[10];		
	char	create_ip[16];				
	char	delete_date[11];			
	char	delete_datetime[10];		
	char	delete_ip[16];				
	char	recover_date[11];			
	char	recover_datetime[10];		
	char	recover_ip[16];				
};
