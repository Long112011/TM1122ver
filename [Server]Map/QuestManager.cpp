		...
		{			
			...
			else
			{
				...
				if( pQuest )
				{
					...
					{
						CNpc* pNpc = (CNpc*)g_pUserTable->FindUser(pmsg->dwFlag);
						if (pNpc == 0){
							if (pmsg->MainQuestIdx == 271 || pmsg->MainQuestIdx == 231)
								return;
							STATIC_NPCINFO* pInfo = GAMERESRCMNGR->GetStaticNpcInfo(pmsg->MainQuestIdx);
							if (pInfo == NULL)
								return;
						...
						else
						{
							if (pNpc->GetNpcJob() == BOMUL_ROLE){
								...
								if (pNpc->GetFirstTalkCharacterID() != pPlayer->GetID())
									return;
							}
							// quest event
							CQuestEvent qe;
