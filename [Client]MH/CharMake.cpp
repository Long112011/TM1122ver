...
BOOL CCharMake::Init(void* pInitParam)
{
	...
	{
		MAP->InitMap(CHARMAKEMGR->GetMakeCfg().wMapNum);//MAP->InitMap(103);
	}
	...
	CHARMAKEMGR->Init();
	CHARMAKEMGR->CreateNewCharacter(&CHARMAKEMGR->GetMakeCfg().vCreatePos);//CHARMAKEMGR->CreateNewCharacter(&g_v3CreatePos);	
