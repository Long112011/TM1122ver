...
void CTitanUpgradeDlg::AddTitanPaper(CItem* pItem)
{
	...
	if( pItem->GetDurability() != 0 && !ITEMMGR->IsDupItem(pItem->GetItemIdx()) )
	{
		...
		ITEMMGR->SetToolTipIcon((cIcon*)&m_VirtualItem, NULL, ITEMMGR->GetItemRareOption(pItem->GetRareness()));  // 2014-11-15 맒
...
void CTitanUpgradeDlg::ViewMaterialList(TITAN_UPGRADEINFO * pTitanUpgradeInfo)
{
	...
	for( int i = 0; i < pTitanUpgradeInfo->wMaterial_Num; i++ )
	{
		...
		ITEMMGR->SetToolTipIcon( (cIcon*)&m_ViewShowItem[i], NULL, ITEMMGR->GetItemRareOption(m_VirtualItem.GetRareness()),0,ITEMMGR->GetItemStoneOption(m_VirtualItem.GetRareness())); // 2014-11-15 맒
