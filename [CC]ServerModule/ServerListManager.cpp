...
{
	BOOL rt;
	CSimpleCodec codec(pFileName,TRUE,ENCRYPT);
...
{
	Clear();
	CSimpleCodec codec(pFileName,FALSE,ENCRYPT);
...
SERVERINFO* CServerListManager::GetPublicServerInfoByServerNum(DWORD ServerKind,WORD Port, WORD wServerNum) 
{
	SERVERINFO* pInfo = NULL;
	m_PublicServerInfoList.SetPositionHead();	
	while( pInfo = m_PublicServerInfoList.GetData() )
	{
		if(pInfo->wServerKind != ServerKind)
			continue;
		if(pInfo->wServerNum == wServerNum && pInfo->wPortForUser==Port)
			return pInfo;
	}
	return NULL;
}
