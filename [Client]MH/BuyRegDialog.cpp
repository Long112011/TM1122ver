...
CBuyRegDialog::CBuyRegDialog()
{
	...
	m_pVolumeEdit	=	NULL;
	m_pMoneyEdit	=	NULL;
	m_pGoldEdit     =   NULL;   //CoNaN 2017-09-15 ‘™±¶±‡º≠øÚ≥ı ºªØ
	m_pRegBtn		=	NULL;
}
...
void CBuyRegDialog::Linking()
{
	...
	m_pGoldEdit	= (cEditBox*)GetWindowForID(BRS_GOLDEDIT);   //CoNaN 2017-09-15 ‘™±¶±‡º≠øÚ…Ë÷√
...
void CBuyRegDialog::UpdateType()
{
	...
	{
		m_pItemList[m_nType]->SetActive(FALSE);					//º±≈√µ«æÓ ¿÷¥¯ æ∆¿Ã≈€ ∏ÆΩ∫∆Æ∏¶ ∫Ò»∞º∫»≠

		m_nType = m_pTypeList->GetCurSelectedRowIdx();			//º±≈√µ» æ∆¿Ã≈€ ¡æ∑˘∏¶ ∞°¡Æø¬¥Ÿ

		m_nItem = 0;											//æ∆¿Ã≈€ ∏ÆΩ∫∆Æ π¯»£ √ ±‚»≠
		m_pItemList[m_nType]->SetCurSelectedRowIdx(m_nItem);

		m_pItemList[m_nType]->SetActive(TRUE);					//º±≈√µ» æ∆¿Ã≈€ ∏ÆΩ∫∆Æ »∞º∫»≠
				
		//º±≈√µ» æ∆¿Ã≈€ ∏ÆΩ∫∆Æ∞° µÓ±ﬁ¿Ã æ¯¥¬ æ∆¿Ã≈€ ¡æ∑˘¿Ã∏È µÓ±ﬁ ∏ÆΩ∫∆Æ∏¶ ∫Ò»∞º∫»≠
...
{
	//±‚¡∏ø° º±≈√µ«æÓ¡¯ æ∆¿Ã≈€∞˙ ¥Ÿ∏• æ∆¿Ã≈€¿Ã º±≈√ µ«æ˙¥Ÿ∏È
	//º±≈√∞™¿ª πŸ≤Ÿ∞Ì µÓ±ﬁ ∏ÆΩ∫∆Æ∏¶ ∏Æº¬ «—¥Ÿ
	if(m_nItem != m_pItemList[m_nType]->GetCurSelectedRowIdx())
	{
		...
		m_nMoney = 0;
		m_pMoneyEdit->SetEditText( DEFAULT_MONEY_TEXT );
		m_pGoldEdit->SetEditText(DEFAULT_MONEY_TEXT);      //CoNaN 2017-09-15 ‘™±¶ƒ¨»œ÷µ…Ë÷√
		m_pVolumeEdit->SetEditText( AddComma(m_nVolume) );
	}
}
...
void CBuyRegDialog::UpdateClass()
{
	if( m_nClass != m_pClassList->GetCurSelectedRowIdx() )
	{
		...
		m_nGold=0;
		m_pGoldEdit->SetEditText( DEFAULT_MONEY_TEXT );  //CoNaN  2017-09-15  ‘™±¶º€∏Ò≥ı ºªØ 
	}
}
...
void CBuyRegDialog::InitRegInfo()
{
	...
	m_nVolume = 0;
	m_nMoney = 0;
	m_nGold=0;      //CoNaN 2017-09-15 ≥ı ºªØ‘™±¶º€∏Ò
...
void CBuyRegDialog::SetRegInfo(BUY_REG_INFO& RegInfo)
{
	...
	m_nGold  =RegInfo.Gold;     //CoNaN 2017-09-15 ‘™±¶º€∏Ò…Ë÷√

	m_pVolumeEdit->SetEditText( AddComma( m_nVolume ) );
	m_pMoneyEdit->SetEditText( AddComma( m_nMoney ) );
	m_pGoldEdit->SetEditText( AddComma( m_nGold ) );  //CoNaN 2017-09-15 ‘™±¶º€∏Ò…Ë÷√
...
BOOL CBuyRegDialog::GetBuyRegInfo(BUY_REG_INFO& BuyInfo)
{
	...
	m_nGold =  (DWORD)atol(RemoveComma(m_pGoldEdit->GetEditText()));    //CoNaN 2017-09-15 ªÒ»°‘™±¶º€∏Ò

	if(m_nVolume < 1)	// ºˆ∑Æ¿Ã 1∞≥ πÃ∏∏¿œ∂ß ø¿∑˘ ∏ﬁºº¡ˆ
	{
		cMsgBox* pMsgBox = WINDOWMGR->MsgBox(MBI_BUYREG, MBT_OK, CHATMGR->GetChatMsg(1209));
		return FALSE;
	}
	else if(m_nVolume > 20)	// ºˆ∑Æ¿Ã 20∞≥ ¿ÃªÛ¿œ∂ß ø¿∑˘ ∏ﬁºº¡ˆ
	{
	...
	else if( m_nMoney < 1)	// ±›æ◊¿Ã 0ø¯ ¿Ã«œ¿œ∂ß ø¿∑˘ ∏ﬁºº¡ˆ
	{
	...
	else if( m_nGold < 0)	//CoNaN 2017-09-15 ‘™±¶º€∏Ò∫œ∑®–‘≈–∂œ
	{
	...
	else if( m_nMoney > 1000000000 )	//±›æ◊¿Ã 10æÔ ¿ÃªÛ¿œ∂ß ø¿∑˘ ∏ﬁºº¡ˆ
	{
	...
	else if( m_nGold > 10000000 )	//CoNaN 2017-09-15 ‘™±¶º€∏Ò≤ªƒ‹≥¨π˝1000ÕÚ 
	{
	...
	BuyInfo.Gold  = m_nGold;    //CoNaN 2017-09-15 ‘™±¶º€∏Ò∏Ωº” 
