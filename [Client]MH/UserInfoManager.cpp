...
void CUserInfoManager::LoadUserInfo( DWORD dwFlag )
{
	...
	if (dwFlag & eUIK_RECONNECT)
	{
		wsprintf(strFilePath, "%s\\Data\\%d\\%d.UIR", DIRECTORYMGR->GetFullDirectoryName(eLM_Root), m_dwSaveFolderName, m_dwSaveFolderName);
		LoadReconnectInterface(strFilePath);
	}
}
void CUserInfoManager::SaveUserInfo( DWORD dwFlag )
{
	...
	{
		wsprintf(strFilePath, "%s\\Data\\%d\\%d.UIR", DIRECTORYMGR->GetFullDirectoryName(eLM_Root), m_dwSaveFolderName, m_dwSaveFolderName);
		SaveReconnectInterface(strFilePath);
	}
}
void CUserInfoManager::LoadReconnectInterface(char* strFilePath)
{
	...
	//	return;
	//}
	//if (bLowResolution != GAMERESRCMNGR->GetResolutionIndex())
	//{
	//	CloseHandle(hFile);
	//	return;
	//}
	...
	//----------------------------------------------------
	pDlg = WINDOWMGR->GetWindowForID(RECONNECT_DLG);
	if (IsValidInterfacePos(pDlg->GetCaptionRect(), UII.DIALOG_INFO[eSD_Reconnect].lPosX, UII.DIALOG_INFO[eSD_Reconnect].lPosY))
		pDlg->SetAbsXY(UII.DIALOG_INFO[eSD_Reconnect].lPosX, UII.DIALOG_INFO[eSD_Reconnect].lPosY);
	//----------------------------------------------------
}
void CUserInfoManager::LoadInterfaceInfo(char* strFilePath)
{
	...
	pDlg = WINDOWMGR->GetWindowForID(ITEM_LINK_TOOLTIP_DLG);
	if (IsValidInterfacePos(pDlg->GetCaptionRect(), UII.DIALOG_INFO[eSD_ItemLink].lPosX, UII.DIALOG_INFO[eSD_ItemLink].lPosY))
		pDlg->SetAbsXY(UII.DIALOG_INFO[eSD_ItemLink].lPosX, UII.DIALOG_INFO[eSD_ItemLink].lPosY);

	//pDlg = WINDOWMGR->GetWindowForID(RECONNECT_DLG);
	//if (IsValidInterfacePos(pDlg->GetCaptionRect(), UII.DIALOG_INFO[eSD_Reconnect].lPosX, UII.DIALOG_INFO[eSD_Reconnect].lPosY))
	//	pDlg->SetAbsXY(UII.DIALOG_INFO[eSD_Reconnect].lPosX, UII.DIALOG_INFO[eSD_Reconnect].lPosY);
	
	pDlg = WINDOWMGR->GetWindowForID(FPS_DLG);
	if (m_bMapChange)
	{
		if ((DWORD)pDlg->IsMovable() != UII.DIALOG_INFO[eSD_FPSCounter].dwFlag & eUIF_ACTIVE)
		{
			pDlg->SetMovable(UII.DIALOG_INFO[eSD_FPSCounter].dwFlag & eUIF_ACTIVE);
			GAMEIN->GetFPSDialog()->SetPinned(UII.DIALOG_INFO[eSD_FPSCounter].dwFlag & eUIF_ACTIVE);
		}			
	}
	if (IsValidInterfacePos(pDlg->GetCaptionRect(), UII.DIALOG_INFO[eSD_FPSCounter].lPosX, UII.DIALOG_INFO[eSD_FPSCounter].lPosY))
		pDlg->SetAbsXY(UII.DIALOG_INFO[eSD_FPSCounter].lPosX, UII.DIALOG_INFO[eSD_FPSCounter].lPosY);
	...
	if (pParty)
	{
		...
		if (m_bMapChange)
		{
			if (UII.DIALOG_INFO[eSD_Party].dwFlag & eUIF_ACTIVE)
				pParty->SetOptionActive(FALSE);
			else
				pParty->SetOptionActive(TRUE);
			if (UII.DIALOG_INFO[eSD_Party].dwParam & eUIF_ACTIVE)
...
void CUserInfoManager::SaveReconnectInterface(char* strFilePath)
{
	...
	if (pDlg)
	{
		UII.DIALOG_INFO[eSD_Reconnect].lPosX = (LONG)pDlg->GetAbsX();
		UII.DIALOG_INFO[eSD_Reconnect].lPosY = (LONG)pDlg->GetAbsY();
	}
	//---------------------------------------------------------------------------
	...
	//int bLowResolution = GAMERESRCMNGR->GetResolutionIndex();
...
void CUserInfoManager::SaveInterfaceInfo( char* strFilePath )
{
	...
	if (pDlg)
	{
		UII.DIALOG_INFO[eSD_ItemLink].lPosX = (LONG)pDlg->GetAbsX();
		UII.DIALOG_INFO[eSD_ItemLink].lPosY = (LONG)pDlg->GetAbsY();
	}

	pDlg = WINDOWMGR->GetWindowForID(FPS_DLG);
	if (pDlg)
	{
		UII.DIALOG_INFO[eSD_FPSCounter].lPosX = (LONG)pDlg->GetAbsX();
		UII.DIALOG_INFO[eSD_FPSCounter].lPosY = (LONG)pDlg->GetAbsY();
		if (pDlg->IsMovable())	UII.DIALOG_INFO[eSD_FPSCounter].dwFlag = eUIF_ACTIVE;
		else					UII.DIALOG_INFO[eSD_FPSCounter].dwFlag = 0;
	}
	...
	if (pDlg)
	{
		...
		*/
		...
		if (pDlg)
		{
			...
			if (pDlg->IsActive())
			{
				UII.DIALOG_INFO[eSD_EventNotify].dwFlag = eUIF_ACTIVE;
...
#ifdef  _MUTIPET_
		pDlg = (cDialog*)PETMGR->GetPetStateDlg();//µ¶¸ç  3pet
		if (pDlg)
		{
			UII.DIALOG_INFO[eSD_PetInfo].lPosX = (LONG)pDlg->GetAbsX();
			UII.DIALOG_INFO[eSD_PetInfo].lPosY = (LONG)pDlg->GetAbsY();

			UII.DIALOG_INFO[eSD_PetInfo].dwParam = TRUE;//PETMGR->GetPetDlgToggle();//µ¶¸ç  3pet
		}
#else
...
void CUserInfoManager::LoadUserState( char* strFilePath )
{
	...
	if( m_bMapChange && HERO )
	{
		...
		HERO->SetKyungGongGrade( UIS.wKyungGongIdx );	
...
BOOL CUserInfoManager::IsValidInterfacePos( RECT* prcCaption, LONG lX, LONG lY )
{
	...
	//if( lX + prcCaption->left < 0 )		return FALSE;
	//else if( lX + prcCaption->right > (LONG)(dispInfo.dwWidth) ) return FALSE;
	//if( lY + prcCaption->top < 0 )		return FALSE;
	//else if( lY + prcCaption->bottom > (LONG)(dispInfo.dwHeight) ) return FALSE;
