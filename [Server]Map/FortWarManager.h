...
protected:
	...
	CPlayer*		m_pEngravePlayerForReward;
	...
	BOOL		dwEngraveNpcRandomPos;
	BOOL		dwEngraveWarSoloMode;
	BOOL		dwEngraveSoloReward;
	DWORD		dwEngraveSoloRewardItemIdx;
	DWORD		dwEngraveSoloRewardItemCount;
	BOOL		dwAllPlayerSameNickName;
	char		dwCustomPlayerNickName[MAX_NAME_LENGTH + 1];
protected:
...
public:
	...
	void ProcessFortWar();
	void LoadRegenPoint();
	void rewardSolo(CPlayer* pPlayer);
	...
	void InitNewNpcKind();
	void ReloadWarState();
	void sendCustomNickNameToClient(CPlayer* pPlayer);

	void FortWarStartTest();
	void FortWarEndTest();
protected:
