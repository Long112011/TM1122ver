#include "stdafx.h"
#include "ChannelSystem.h"
#include "MHFile.h"
#include "BattleSystem_Server.h"
#include "Object.h"
#include "NetWork.h"
#include "EventMapMgr.h"
#include "QuestMapMgr.h"

#include "PKManager.h"
#include "dungeon\DungeonMgr.h"

#include "InsDungeonManager.h"
#include "InsDungeonMapManager.h"
#include "Player.h"

extern int g_nServerSetNum;

CChannelSystem::CChannelSystem()
{
	m_dwCount = 0;
	m_bQuestMap = FALSE;
	m_bFubenMap=FALSE;
	memset( m_QMChannel, 0, sizeof(CChannel)*1000 );
	memset( m_FBChannel, 0, sizeof(CChannel)*1000 );
	memset( m_Channel, 0, sizeof(CChannel*)*MAX_CHANNEL_NUM );

	m_bInsDungeonMap = FALSE;
	m_InsDGChannelTable.Initialize(100);
}

CChannelSystem::~CChannelSystem()
{
	for(DWORD i=0; i<m_dwCount; ++i)
	{
		if(m_Channel[i])
		{		
			delete m_Channel[i];
			m_Channel[i] = NULL;
		}
	}
}

void CChannelSystem::LoadChannelInfo()
{
	CMHFile file;
	BYTE Kind, num = 0;
	char value[64] = {0,};
	WORD wMapNum = 0;
	
	char filename[256];
	sprintf(filename,"serverset/%d/ChannelInfo.bin",g_nServerSetNum);

	file.Init(filename, "rb");

	if(file.IsInited() == FALSE)
	{
		MessageBox(NULL,"Can't Open ChannelInfo File", NULL, NULL);
		return;
	}

	//Load default
	char buf[1024];
	file.GetLine(buf, 1024);		//주석
	CHANNEL_INFO defaultInfo;
	defaultInfo.RegenRate = file.GetFloat();
	defaultInfo.DropRate = file.GetFloat();
	defaultInfo.bBattleChannel = file.GetBool();
	defaultInfo.bPKAllowChannel= file.GetBool();
	defaultInfo.ExpRate = file.GetFloat();
	defaultInfo.AbilityExpRate = file.GetFloat();
	defaultInfo.MoneyRate  =file.GetFloat();
	file.GetLine(buf, 1024);		//주석

	char temp[1024] = {0,};
	char line[512] = {0,};
	while(!file.IsEOF())
	{
		file.GetString(temp);
		//SafeStrCpy( temp, file.GetString(), 1024 );

		if( temp[0] == '/' )
		{
			file.GetLineX(line,512);
			continue;
		}

		if( '#' == temp[0] )
		{
			if( atoi(temp+1) == g_pServerSystem->GetMapNum() )
			{
				Kind = file.GetByte();
				num = file.GetByte();
				CreateChannel(Kind, num);

				for( int i = 0; i < num; ++i)
				{
					m_Channel[i]->m_ChannelInfo.RegenRate		= defaultInfo.RegenRate;
					m_Channel[i]->m_ChannelInfo.DropRate		= defaultInfo.DropRate;

					m_Channel[i]->m_ChannelInfo.bBattleChannel	= defaultInfo.bBattleChannel;
					m_Channel[i]->m_ChannelInfo.bPKAllowChannel = defaultInfo.bPKAllowChannel;

					m_Channel[i]->m_ChannelInfo.ExpRate			= defaultInfo.ExpRate;
					m_Channel[i]->m_ChannelInfo.AbilityExpRate	= defaultInfo.AbilityExpRate;
					m_Channel[i]->m_ChannelInfo.MoneyRate		= defaultInfo.MoneyRate;
				}

				DWORD ChannelCnt = 0;
				DWORD ChannelIDX = 0;
				while(!file.IsEOF())
				{
					++ChannelCnt;

					ChannelIDX = file.GetDword();

					if( m_dwCount >= ChannelIDX && ChannelCnt == ChannelIDX )
					{
						file.GetFloat();
						m_Channel[ChannelIDX - 1]->m_ChannelInfo.RegenRate			= 1.0f;
						m_Channel[ChannelIDX - 1]->m_ChannelInfo.DropRate			= file.GetFloat();

						m_Channel[ChannelIDX - 1]->m_ChannelInfo.bBattleChannel		= file.GetBool();
						m_Channel[ChannelIDX - 1]->m_ChannelInfo.bPKAllowChannel	= file.GetBool();

						m_Channel[ChannelIDX - 1]->m_ChannelInfo.ExpRate			= file.GetFloat();
						m_Channel[ChannelIDX - 1]->m_ChannelInfo.AbilityExpRate		= file.GetFloat();
						m_Channel[ChannelIDX - 1]->m_ChannelInfo.MoneyRate			= file.GetFloat();
					}
					else if( m_dwCount >= ChannelCnt )
					{
						m_Channel[ChannelCnt - 1]->m_ChannelInfo.RegenRate			= defaultInfo.RegenRate;
						m_Channel[ChannelCnt - 1]->m_ChannelInfo.DropRate			= defaultInfo.DropRate;

						m_Channel[ChannelCnt - 1]->m_ChannelInfo.bBattleChannel		= defaultInfo.bBattleChannel;
						m_Channel[ChannelCnt - 1]->m_ChannelInfo.bPKAllowChannel	= defaultInfo.bPKAllowChannel;

						m_Channel[ChannelCnt - 1]->m_ChannelInfo.ExpRate			= defaultInfo.ExpRate;
						m_Channel[ChannelCnt - 1]->m_ChannelInfo.AbilityExpRate		= defaultInfo.AbilityExpRate;
						m_Channel[ChannelCnt - 1]->m_ChannelInfo.MoneyRate			= defaultInfo.MoneyRate;

						file.Seek(-2);
					}
					else
					{
						break;
					}
				}
				break;
			}
		}
	}
	file.Release();
	//g_Console.LOG(4, "Channel inited!");


	if(m_dwCount == 0)
		CreateChannel(0, 1);
}

void CChannelSystem::CreateChannel(BYTE Kind, BYTE num)
{
	for(BYTE i=0; i<num; ++i)
	{			
		 DWORD ChannelIDX = BATTLESYSTEM->CreateChannel();
		 m_Channel[i] = new CChannel;
		 m_Channel[i]->SetChannelID(ChannelIDX);
	} 
	
	m_dwCount = num;
	m_bQuestMap = FALSE;
	m_bFubenMap = FALSE;

	m_bInsDungeonMap = FALSE;
}

void CChannelSystem::IncreasePlayerNum(DWORD ChannelID)
{
	if( m_bQuestMap )	return;
	if(m_bFubenMap) return;

	if (m_bInsDungeonMap)	return;

	if(ChannelID > m_dwCount)
	{
		return;
	}
	m_Channel[ChannelID-1]->IncreaseNum();
}

void CChannelSystem::DecreasePlayerNum(DWORD ChannelID)
{
	if( m_bQuestMap )	return;
	if( m_bFubenMap ) return;

	if (m_bInsDungeonMap)	return;

	if(ChannelID > m_dwCount)
	{
		return;
	}
	m_Channel[ChannelID-1]->DecreaseNum();
}


void CChannelSystem::SendChannelInfo(MSG_DWORD2* pInfo, DWORD dwConnectionIndex)
{
	if (g_pServerSystem->GetStart() == FALSE || m_bQuestMap || m_bFubenMap || m_bInsDungeonMap)
	{
		MSG_BYTE nmsg;
		nmsg.Category = MP_USERCONN;
		nmsg.Protocol = MP_USERCONN_CHANNELINFO_NACK;
		nmsg.bData = 0;
		nmsg.dwObjectID = pInfo->dwObjectID;
		g_Network.Send2Server(dwConnectionIndex,(char*)&nmsg,sizeof(nmsg));
		return;
	}

	MSG_CHANNEL_INFO msg;
	memset(&msg, 0, sizeof(MSG_CHANNEL_INFO));
	msg.Category = MP_USERCONN;
	msg.Protocol = MP_USERCONN_CHANNELINFO_ACK;

	for(DWORD i=0; i<m_dwCount; ++i)
	{
		msg.PlayerNum[i] = m_Channel[i]->GetPlayerNum();		
		msg.RegenRate[i]=m_Channel[i]->GetRegenRate();
		msg.DropRate[i]=m_Channel[i]->GetDropRate();
		msg.bBattleChannel[i] = m_Channel[i]->IsBattleChannel();
		msg.bPKAllowChannel[i]=m_Channel[i]->IsPKAllowChannel();
		msg.ExpRate[i]=m_Channel[i]->GetExpRate();
		msg.AbilityExpRate[i]=m_Channel[i]->GetAbilityRate();
		msg.MoneyRate[i]=m_Channel[i]->GetMoneyRate();
	}
	msg.dwObjectID = pInfo->dwObjectID;
	msg.dwUniqueIDinAgent = pInfo->dwData1;
	msg.Count = (BYTE)m_dwCount;
	g_Network.Send2Server(dwConnectionIndex,(char*)&msg,sizeof(msg));
}

DWORD CChannelSystem::GetChannelID(int num)
{
	if( m_bQuestMap )	return 0;
	if( m_bFubenMap ) return 0;

	if (m_bInsDungeonMap)	return 0;

	if(((DWORD)num >= m_dwCount) || (num < 0))
	{
		return 0;
	}
		
	return m_Channel[num]->GetChannelID();
}

void CChannelSystem::SendChannelInfoToMS( DWORD dwConnectionIndex )
{
	if( m_bQuestMap )	return;
	
	MSG_CHANNEL_INFO msg;
	memset(&msg, 0, sizeof(MSG_CHANNEL_INFO));
	msg.Category = MP_MORNITORMAPSERVER;
	msg.Protocol = MP_MORNITORMAPSERVER_QUERY_CHANNELINFO_ACK;

	for(DWORD i=0; i<m_dwCount; ++i)
	{
		msg.PlayerNum[i] = m_Channel[i]->GetPlayerNum();
	}
	msg.Count = (BYTE)m_dwCount;
	g_Network.Send2Server(dwConnectionIndex,(char*)&msg,sizeof(msg));
}

void CChannelSystem::InitQuestMapChannel()
{
	m_bQuestMap = TRUE;
}

DWORD CChannelSystem::CreateQuestMapChannel() 
{
	if( !m_bQuestMap )	return 0;

	DWORD ChannelIDX = BATTLESYSTEM->CreateChannel();
	int i = 0;
	for(i = 0; i < 1000; ++i )
	{
		if( m_QMChannel[i].GetChannelID() == 0 )
			break;
	}
	if( i > 999 )	return 0;

	m_QMChannel[i].SetChannelID( ChannelIDX );
	
	++m_dwCount;

	return ChannelIDX;
}

void CChannelSystem::DestroyQuestMapChannel( DWORD dwChannel )
{
	if( !m_bQuestMap )	return;

	for( int i = 0; i < 1000; ++i )
	{
		if( m_QMChannel[i].GetChannelID() == dwChannel )
		{
			m_QMChannel[i].SetChannelID( 0 );
			break;
		}
	}
	--m_dwCount;

	BATTLESYSTEM->DestroyChannel( dwChannel );	
}

void CChannelSystem::Init( WORD wMapNum )
{
	//if( QUESTMAPMGR->Init( wMapNum ) )	return;
	//if( DungeonMGR->Init(  ) )	return;//kiv
		//if( QUESTMAPMGR->InitFuben( wMapNum ) )	return;
	if (g_pServerSystem->GetMap()->IsMapKind(eQuestRoom))
	{
		QUESTMAPMGR->Init(wMapNum);
		return;
	}
	else if (g_pServerSystem->GetMap()->IsMapKind(eInsDungeon))
	{
		INSDUNGEONMAPMGR->Init(wMapNum);
		return;
	}
	if( wMapNum == EVENTMAPNUM )
	{
		EVENTMAPMGR->Init( wMapNum, 10 );
	}
	else
	{
		LoadChannelInfo();
	}
}

DWORD CChannelSystem::GetRandomChannelID()
{
	if( m_bQuestMap )	return 0;
	if( m_bFubenMap ) return 0;

	if (m_bInsDungeonMap)	return 0;

	int num = 0;
	if( m_dwCount > 1 )
		num = random(0, GetChannelCount()-1);
	return m_Channel[num]->GetChannelID();
}

DWORD CChannelSystem::GetLastChannelID()
{
	if( m_bQuestMap )	return 0;
	if( m_bFubenMap ) return 0;

	if (m_bInsDungeonMap)	return 0;

	return m_Channel[m_dwCount-1]->GetChannelID();
}

void CChannelSystem::SendMapChangeChannelInfo(MSG_DWORD3* pInfo, DWORD dwConnectionIndex)
{
	if (g_pServerSystem->GetStart() == FALSE || m_bQuestMap || m_bFubenMap || m_bInsDungeonMap)
	{
		MSG_BYTE nmsg;
		nmsg.Category = MP_USERCONN;
		nmsg.Protocol = MP_USERCONN_CHANGEMAP_CHANNELINFO_NACK;
		nmsg.bData = 0;
		nmsg.dwObjectID = pInfo->dwObjectID;
		g_Network.Send2Server(dwConnectionIndex,(char*)&nmsg,sizeof(nmsg));
		return;
	}

	MSG_CHANNEL_INFO msg;
	memset(&msg, 0, sizeof(MSG_CHANNEL_INFO));
	msg.Category = MP_USERCONN;
	msg.Protocol = MP_USERCONN_CHANGEMAP_CHANNELINFO_ACK;

	for(DWORD i=0; i<m_dwCount; ++i)
	{
		msg.PlayerNum[i] = m_Channel[i]->GetPlayerNum();		
		msg.RegenRate[i]=m_Channel[i]->GetRegenRate();
		msg.DropRate[i]=m_Channel[i]->GetDropRate();
		msg.bBattleChannel[i] = m_Channel[i]->IsBattleChannel();
		msg.bPKAllowChannel[i]=m_Channel[i]->IsPKAllowChannel(); 
		msg.ExpRate[i]=m_Channel[i]->GetExpRate(); 
		msg.AbilityExpRate[i]=m_Channel[i]->GetAbilityRate(); 
		msg.MoneyRate[i]=m_Channel[i]->GetMoneyRate(); 
	}
	msg.dwObjectID = pInfo->dwObjectID;
	msg.dwUniqueIDinAgent = pInfo->dwData1;
	msg.Count = (BYTE)m_dwCount;
	msg.wMoveMapNum = pInfo->dwData2;
	msg.dwChangeMapState = pInfo->dwData3;
	g_Network.Send2Server(dwConnectionIndex,(char*)&msg,sizeof(msg));
}
float CChannelSystem::GetRegenRate( DWORD ChannelID )
{
	if( m_bQuestMap )	return 1.0f;
	if( m_bFubenMap ) return 1.0f;

	if (m_bInsDungeonMap)	return 1.0f;

	if( !ChannelID || ChannelID > m_dwCount )
	{
		return 1.0f;
	}

	if( m_Channel[ChannelID - 1] == NULL )
		return 1.0f;

	return m_Channel[ChannelID - 1]->GetRegenRate();
}

float CChannelSystem::GetDropRate( DWORD ChannelID )
{
	if( m_bQuestMap )	return 1.0f;
	if (m_bFubenMap) return 1.0f;

	if (m_bInsDungeonMap)	return 1.0f;

	if( !ChannelID || ChannelID > m_dwCount )
	{
		return 1.0f;
	}

	if( m_Channel[ChannelID - 1] == NULL )
		return 1.0f;

	return m_Channel[ChannelID - 1]->GetDropRate();
}

BOOL CChannelSystem::IsBattleChannel( DWORD ChannelID )
{
	if( m_bQuestMap )	return FALSE;
	if( m_bFubenMap ) return FALSE;

	if (m_bInsDungeonMap)	return FALSE;

	if( !ChannelID || ChannelID > m_dwCount )
	{
		return FALSE;
	}

	if( m_Channel[ChannelID - 1] == NULL )
		return FALSE;

	return m_Channel[ChannelID - 1]->IsBattleChannel();
}
BOOL CChannelSystem::IsPKAllowChannel( DWORD ChannelID )
{
	if( m_bQuestMap )	return FALSE;
	if( m_bFubenMap ) return FALSE;

	if (m_bInsDungeonMap)	return FALSE;

	if( !ChannelID || ChannelID > m_dwCount )
	{
		return FALSE;
	}

	if( m_Channel[ChannelID - 1] == NULL )
		return FALSE;

	return m_Channel[ChannelID - 1]->IsPKAllowChannel();
}
void CChannelSystem::GetChannelInfo( MSG_CHANNEL_INFO* pInfo )
{
	if( m_bQuestMap )	return;
	if( m_bFubenMap ) return;

	//if (m_bInsDungeonMap)	return ;

	for( DWORD i = 0; i < m_dwCount; ++i )
	{
		pInfo->bBattleChannel[i] = m_Channel[i]->IsBattleChannel();
		pInfo->bPKAllowChannel[i]= m_Channel[i]->IsPKAllowChannel();

		pInfo->ExpRate[i] = m_Channel[i]->GetExpRate();
		pInfo->AbilityExpRate[i] = m_Channel[i]->GetAbilityRate();
		pInfo->MoneyRate[i] = m_Channel[i]->GetMoneyRate();
	}
	pInfo->Count = m_dwCount;
}


void CChannelSystem::InitFubenMapChannel()
{
	m_bFubenMap = TRUE;
}
DWORD CChannelSystem::CreateFubenMapChannel() 
{
	if( !m_bFubenMap )	return 0;

	DWORD ChannelIDX = BATTLESYSTEM->CreateChannel();
	int i = 0;
	for(i = 0; i < 1000; ++i )
	{
		if( m_FBChannel[i].GetChannelID() == 0 )
			break;
	}
	if( i > 999 )	return 0;

	m_FBChannel[i].SetChannelID( ChannelIDX );
	
	++m_dwCount;

	return ChannelIDX;
}
void CChannelSystem::DestroyFubenMapChannel( DWORD dwChannel )
{
	if( !m_bFubenMap )	return;

	for( int i = 0; i < 1000; ++i )
	{
		if( m_FBChannel[i].GetChannelID() == dwChannel )
		{
			m_FBChannel[i].SetChannelID( 0 );
			break;
		}
	}
	--m_dwCount;

	BATTLESYSTEM->DestroyChannel( dwChannel );
}
void CChannelSystem::InitInsDungeonMapChannel()
{
	m_bInsDungeonMap = TRUE;
}
DWORD CChannelSystem::CreateInsDungeonMapChannel(CPlayer* pPlayer)
{
	if (!m_bInsDungeonMap)
		return 0;

	DWORD dwChannelIDX = 0;
	CChannel* pChannel = NULL;

	//인던 정보가 있는지 체크후 없으면 생성하고 있으면 할당
	DWORD dwInsDungeonChannel = INSDUNGEONMGR->GetInsDungeonChennel(pPlayer);

	if (dwInsDungeonChannel == 0)	//채널 정보가 없으면 채널 생성
	{
		dwChannelIDX = BATTLESYSTEM->CreateChannel();
		pChannel = new CChannel;
		pChannel->SetChannelID(dwChannelIDX);
		m_InsDGChannelTable.Add(pChannel, dwChannelIDX);

		//채널정보 생성 후 인던 생성
		BOOL bSuccess = INSDUNGEONMGR->CreateInsDungeon(dwChannelIDX, pPlayer);
		if (bSuccess == FALSE)
		{
			ASSERT(0);
			return 0;
		}
	}
	else	//채널 정보가 있으면 해당 채널 셋팅
	{
		dwChannelIDX = dwInsDungeonChannel;
	}

	++m_dwCount;

	return dwChannelIDX;
}

void CChannelSystem::DestroyInsDungeonMapChannel(DWORD dwChannel)
{
	if (!m_bInsDungeonMap)
		return;

	CChannel* pChannel = m_InsDGChannelTable.GetData(dwChannel);
	SAFE_DELETE(pChannel);
	m_InsDGChannelTable.Remove(dwChannel);

	--m_dwCount;

	BATTLESYSTEM->DestroyChannel(dwChannel);
	BATTLESYSTEM->ReleaseBattleID(dwChannel);
}
float CChannelSystem::GetExpRate( DWORD ChannelID )
{
	if( m_bQuestMap )	return 1.0f;
	if( m_bFubenMap ) return 1.0f;

	if (m_bInsDungeonMap)	return 1.0f;

	if( !ChannelID || ChannelID > m_dwCount )
	{
		return 1.0f;
	}

	if( m_Channel[ChannelID - 1] == NULL )
		return 1.0f;

	return m_Channel[ChannelID - 1]->GetExpRate();
}
float CChannelSystem::GetAbilityRate( DWORD ChannelID )
{
	if( m_bQuestMap )	return 1.0f;
	if( m_bFubenMap ) return 1.0f;

	if (m_bInsDungeonMap)	return 1.0f;

	if( !ChannelID || ChannelID > m_dwCount )
	{
		return 1.0f;
	}

	if( m_Channel[ChannelID - 1] == NULL )
		return 1.0f;

	return m_Channel[ChannelID - 1]->GetAbilityRate();
}
float CChannelSystem::GetMoneyRate( DWORD ChannelID )
{
	if( m_bQuestMap )	return 1.0f;
	if( m_bFubenMap ) return 1.0f;

	if (m_bInsDungeonMap)	return 1.0f;

	if( !ChannelID || ChannelID > m_dwCount )
	{
		return 1.0f;
	}

	if( m_Channel[ChannelID - 1] == NULL )
		return 1.0f;

	return m_Channel[ChannelID - 1]->GetMoneyRate();
}


DWORD CChannelSystem::GetPlayerNumInChannel(int index)
{
	if(DungeonMGR->IsDungeon(g_pServerSystem->GetMapNum()))
	{
		return DungeonMGR->GetPlayerNumInDungeon(index);
	}
	else if(QUESTMAPMGR->IsQuestMap())
	{
		DWORD dwPlayerNum = m_QMChannel[index-1].GetPlayerNum();
		return dwPlayerNum;
	}

	--index;

	if( int(m_dwCount) <= index )
	{
		return 0;
	}

	return m_Channel[index]->GetPlayerNum();
}