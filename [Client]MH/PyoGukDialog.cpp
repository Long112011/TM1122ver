...
void CPyogukDialog::Linking()
{
	...
	for(int i=0; i<MAX_PYOGUKLIST_NUM; ++i)
	{
		...
		m_PyogukCtlListArray[ePyogukMode_PyogukWare1+i].AddTail((cStatic*)GetWindowForID(PYO_MONEYIMG)); //µø¿¸ ±◊∏≤
		m_PyogukCtlListArray[ePyogukMode_PyogukWare1+i].AddTail((cStatic*)GetWindowForID(PYO_MONEYBACKIMG));
		m_PyogukCtlListArray[ePyogukMode_PyogukWare1+i].AddTail(m_pMoney); //πŸ≈¡ ¿ÃπÃ¡ˆ
...
#else
		sprintf(ritem->pString[3], "%s", AddComma(m_PyogukListInfo.Pyoguk[i].BuyPrice));   //weiye   ≤÷ø‚ºÚΩÈÃÌº”µΩ¡–±Ì  2018-04-07
#endif
...
{//[”“º¸≤÷¥¢][BY: Æ¿Ô∆¬Ω£¥´∆Ê][QQ:112582793][2019-4-16][18:18:03]
	if(m_CurPyogukMode >= ePyogukMode_NoWare)
	{
		char buf[128];
		sprintf(buf, "CurPyogukMode : %d", m_CurPyogukMode);
...
{//[”“º¸≤÷¥¢][BY: Æ¿Ô∆¬Ω£¥´∆Ê][QQ:112582793][2019-4-16][18:18:03]
	...
	if(m_CurPyogukMode == 0)
		return;
	m_PyogukItemDlg[m_CurPyogukMode-1]->MoveItemToPyoguk(pFromItem,(m_CurPyogukMode-1)*30);
}