	...
	while(pos)
	{
		...
		if(pConditionInfo->bRegen)
		{
			if(pConditionInfo->dwStartRegenTick + pConditionInfo->dwRegenDelay < gCurTime)
			{
				...
				//if(DungeonMGR->SuspendMonster(0))
				//	continue;

				//if(DungeonMGR->SuspendMonster(m_pGroup->GetGridID()))
				//	continue;

				if(FORTWARMGR->SuspendMonster(m_pGroup->GetGridID()))
					continue;

				if(GUILDWARMGR->SuspendMonster(m_pGroup->GetGridID()))
					continue;
