...
ITEMBASE /*const*/ * CItemSlot::GetItemInfoAbs(POSTYPE absPos)//kiv const
{ 
...
ERROR_ITEM CItemSlot::UpdateItemAbs(CPlayer * pPlayer, POSTYPE whatAbsPos, DWORD dwDBIdx, WORD wItemIdx, POSTYPE position, POSTYPE quickPosition, DURTYPE Dur, WORD itemstatic, WORD ItemQuality, WORD ItemEntry1, WORD ItemEntry2, WORD ItemEntry3, WORD flag, WORD state, DWORD RareDBIdx,DWORD StoneIdx,DWORD Grow,char * PowerUp,char * Green, DWORD Grade30)
{
	...
	if(flag & UB_STONE)
	m_ItemBaseArray[whatAbsPos].StoneIdx=StoneIdx; // 2014-11-14 Êý¾Ý!

	if(flag & UB_LOCK)
	m_ItemBaseArray[whatAbsPos].ItemStatic = ITEM_STATIC_LUCK;  // 

	if(flag & UB_UNLOCK)
	m_ItemBaseArray[whatAbsPos].ItemStatic = 0;  // ½âËø

	if(flag & UB_GROW)
	m_ItemBaseArray[whatAbsPos].ItemGrow  =Grow;    // ³É³¤Êý¾Ý¸üÐÂ! 2015-01-15

	if(flag & UB_POWERUP)
		strcpy(m_ItemBaseArray[whatAbsPos].PowerUp,PowerUp);
	...
	// magi82 - ³ëÁ¡»ó ÇÑ¹øÀÌ¶óµµ ³¹°³·Î ÆÈ¸é ±×´ÙÀ½ºÎÅÍ ÅëÂ°·Î ÆÈ¾ÆÁöÁö¾Ê´Â ¹ö±× ¼öÁ¤
...
/* PositionÀÌ ÇöÀç Slot¿¡ Æ÷ÇÔµÇ´Â PositionÀÎÁö Check*/
