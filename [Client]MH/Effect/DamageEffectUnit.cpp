...
BOOL CDamageEffectUnit::On(EFFECTPARAM* pParam)
{
...

	{
		...
		if(MainTargetID && MainTargetID != HEROID)
		{
			...
			if(pMainTarget)
			{
				//HERO->OnHitTarget(pMainTarget);//kiv //default off 
...
DWORD CalcTotalDamage(DWORD CurTotal,float CurRate,float SumRate)
{
	...
	float inverse = 1 / (1 - lateSum);//float inverse = 1.f / (1.f - lateSum);//kiv
...
{
	//if( pResultInfo->RealDamage > 1000000/*10000000000*/ )//1000000//kiv
	//{
	//	ASSERT(0);
	//	return;
	//}
