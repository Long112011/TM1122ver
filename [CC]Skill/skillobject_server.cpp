...
DWORD CSkillObject::Update()
{
	...
	////////////////////////////////////////////////////////////////////////////////////////////
	// 06. 04. 스킬 버그 수정 - 이영준
	// 부착(지) 이펙트를 가진 스킬이면서 범위형 스킬이 아닌경우는
...
BOOL CSkillObject::TargetTypeCheck(WORD TargetType,CObject* pObject)
{
	...
	case SKILLTARGETTYPE_ENEMY:
		{
			CBattle* pBattle = pObject->GetBattle();	// m_pOperator 한 베틀을 얻어온다.
	...
	case SKILLTARGETTYPE_NOTENEMYTARGET:
		{
			CBattle* pBattle = BATTLESYSTEM->GetBattle(m_pOperator->GetBattleID());	// m_pOperator 한 베틀을 얻어온다.
			if(pBattle->IsEnemy(m_pOperator,pObject))
				return FALSE;
...
BOOL CSkillObject::IsSame(CSkillObject* pSkill)
{
	...
	// 2. 성능력 증폭 스킬은 모두 같은 계열이다. - 무조건 치고 들어간다
