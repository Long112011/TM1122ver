...
float CPlayer::DoGetMoveSpeed()
{
	...
	else
	{
		...
		if (m_MoveInfo.KyungGongIdx)
		{
			...
			// ====== ´Ì¿Í£¨WeaponType == 11£©Ãô½Ý¼ÓËÙÂß¼­ ======
			WORD weaponType = GetWeaponEquipType();
			if (weaponType == 11)
			{
				int minChub = GetMinChub(); // ´Ó×ÔÉí»ñÈ¡Ãô½Ý£¨´©´Ì£©Öµ
				speed += (float)(minChub / 3); // Ã¿ 3 µã¼Ó 1 ËÙ¶È
			}
			// =========================================
...
DWORD CPlayer::DoGetPhyDefense()//ÎïÀí·ÀÓù
{ 
	/*// ¹æ¾î °ø½Ä µû·Î Ã³¸®.
	//SW070127 Å¸ÀÌÅº
	if(InTitan())
...
void CPlayer::SetMallMoney(MONEYTYPE ChangeValue, BYTE nFlag, DWORD ItemIdx)
{
	...
	if (nFlag == 1 || nFlag == 3 || nFlag == 6)//ÏûºÄÅÝµã
	{
		if (HeroTotalMallMoney < ChangeValue)
		{
			g_Console.LOGANDFILE(".\\ServerSet\\ÖØÒªÈÕÖ¾\\ÖØÒªÈÕÖ¾.txt",
				RGB(0, 0, 255), "ÅÝµã¿Û¼õÊý¾ÝÒì³£ ½ÇÉ«ID[%d] ½ÇÉ«Ãû[%s] Ôª±¦Êý[%d]",
	...
	if (nFlag == 2 || nFlag == 4 || nFlag == 5)//»ñµÃÅÝµã
	{
		LastMallMoney = HeroTotalMallMoney + ChangeValue;
	}
	if (LastMallMoney < 0)
...
void CPlayer::SetGoldMoney(MONEYTYPE ChangeValue, BYTE nFlag, WORD ItemIdx, WORD ItemDurability)
{
	...
	if (nFlag == eShopUse || nFlag == eNoneItem || nFlag == eFbUse || nFlag == eDeleteBuf || nFlag == eFame || nFlag == eSinged)
	{
		...
		{
			g_Console.LOGANDFILE(".\\ServerSet\\ÖØÒªÈÕÖ¾\\ÖØÒªÈÕÖ¾.txt",
				RGB(0, 0, 255), "Ôª±¦¿Û¼õÊý¾ÝÒì³£ ½ÇÉ«ID[%d] ½ÇÉ«Ãû[%s] Ôª±¦Êý[%d]",
				GetID(), GetObjectName(), ChangeValue);
			...
			CharacterHeroGoldInfoUpdate(GetID(), m_HeroInfo.GoldMoney, nFlag);//ÔÚÏß³äÖµÔª±¦Ë¢ÐÂ
	...
	msgMoney.bFlag = nFlag;
	SendMsg(&msgMoney, sizeof(msgMoney));
	CharacterHeroGoldInfoUpdate(GetID(), ChangeValue, nFlag);//µ¥¶ÀÔª±¦Ë¢ÐÂ
}
void CPlayer::UpdateGoldMoney(DWORD DBGoldMoney, DWORD ChangeMoney, BOOL bIsSend)//ÔÚÏß³äÖµÔª±¦Ë¢ÐÂ
{
	m_HeroInfo.GoldMoney = DBGoldMoney;
	if (bIsSend)
	{
...
void CPlayer::GetDBGoldMoney()  //ÔÚÏß³äÖµÔª±¦Ë¢ÐÂ
{
	char txt[64];
...
void CPlayer::SetVipLevel(DWORD Level)
{
	...
	msg.dwData = Level;
	SendMsg(&msg, sizeof(msg));
	CharacterHeroInfoUpdate(this);//¸üÐÂVipµÈ¼¶  Ê¹ÓÃÈ«¾Ö¸üÐÂ
}
