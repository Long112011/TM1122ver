	...
	case MP_USERCONN_LOGIN_SYN:
		{
			...
			SafeStrCpy(pUserInfo->Mac,pmsg->Mac,18);

			// 

			pUserInfo->wServerNum=pmsg->ServerNum;
...
//---
			...
			*/
			...
			if( pmsg->gate == 1 )
				...
				LoginCheckQueryEx(bufIDMD5, bufPW, ip, g_nServerSetNum, pmsg->AuthKey, dwConnectionIndex, g_dwMaxUser, FastInfo->wServerNum, bufPN,pmsg->Mac,g_dwMaxClientUser);
			else
				LoginCheckQueryEx(bufID, bufPW, ip, g_nServerSetNum, pmsg->AuthKey, dwConnectionIndex, g_dwMaxUser, FastInfo->wServerNum, bufPN,pmsg->Mac,g_dwMaxClientUser);
#else
	...
	case MP_USERCONN_FORCE_DISCONNECT_OVERLAPLOGIN:
		{
			...
			//pUserInfo->State == 4 이어야지�?중복 로그�?체크가 온것이다.
			//아니라면 가짜로 보낸 �? 잘라버리�?
...
//			g_pLoginUser.SetCount( pUserInfo->dwUserID, g_pServerTable->GetDataNum() );
	...
	case MP_USERCONN_NOTIFY_USERLOGIN_NACK:		//KKK
		{	// Agent 에서 온다. 1.에이젼트서버가 안켜�?있을�?2.중국빌링돈없을때
			MSG_DWORD3* pmsg = (MSG_DWORD3*)pMsg;
			...
			if( pInfo->dwUniqueConnectIdx != DistAuthKey )	//AAA 이미 다른 사람�?들어 왔어!
				return;
	...
	case MP_USERCONN_REGUSER_SYN:
		{
			...
			WORD wPort;
			g_Network.GetUserAddress( dwConnectionIndex,pInfo->IP, &wPort );
			...
			SafeStrCpy(pInfo->Mac,pmsg->Mac,18);
			RegQueryEx(dwConnectionIndex);
		}
		break;
	case MP_USERCONN_RESTPWD_SYN:
		{
									//g_Console.LOC(0xFF0000, 4, "distribute MP_USERCONN_RESTPWD_SYN");
			MSGREG* pmsg = (MSGREG*)pMsg;
			USERINFO* pInfo = g_pUserTable->FindUser(dwConnectionIndex);
			if(pInfo==NULL)
			{
