...
void CGuild::SetHuntedMonsterTotalCountInfo( int SetTotal, QSTATETYPE DBProcessTime )
{
...
void CGuild::GuildWarSprotf(GUILD_WAR_RAND_INFO* pSprotfInfo)
{
	if (pSprotfInfo->pGuildPoint)
	{
		GuildAddGuildPoint(GetIdx(),pSprotfInfo->pGuildPoint,eGuildWar,GetIdx());
	}
	CPlayer* pPlayer =NULL;
	PTRLISTPOS pos = m_MemberList.GetHeadPosition();
	while(pos)
	{
		...
		if( pInfo )
		{
			pPlayer = (CPlayer*)g_pUserTable->GetData(pInfo->MemberIdx);
			if(pPlayer)
			{
				if(pInfo->Rank == GUILD_MASTER)
				{//쳔寮쉽쟨
					if(pSprotfInfo->pMaster.pMoneyJB)
					{
						pPlayer->SetGoldMoney(pSprotfInfo->pMaster.pMoneyJB,eGetGoldMoney);
					}
					if(pSprotfInfo->pMaster.pMoneyPD)
					{
						pPlayer->SetMallMoney(pSprotfInfo->pMaster.pMoneyPD,2);
					}
					if(pSprotfInfo->pMaster.pMoney)
					{
						pPlayer->SetMoney(pSprotfInfo->pMaster.pMoney,MONEY_ADDITION);
					}
				}
				else
				{//페儉냥逃쉽
					if(pSprotfInfo->pOther.pMoneyJB)
					{
						pPlayer->SetGoldMoney(pSprotfInfo->pOther.pMoneyJB,eGetGoldMoney);
					}
					if(pSprotfInfo->pOther.pMoneyPD)
					{
						pPlayer->SetMallMoney(pSprotfInfo->pOther.pMoneyPD,2);
					}
					if(pSprotfInfo->pOther.pMoney)
					{
						pPlayer->SetMoney(pSprotfInfo->pOther.pMoney,MONEY_ADDITION);
					}
				}
			}
		}
...
WORD CGuild::ChangeMaster(CPlayer* pMaster,DWORD pChangeIDX,char* pChangeName)
{
	CPlayer* pPlayer =NULL;

	if(!IsMember(pChangeIDX))
		return 1;
	if (GetMasterIdx() != pMaster->GetID())
		return 2;

	GUILDMEMBERINFO* pChangeInfo = GetMemberInfo(pChangeIDX);
	if(!pChangeInfo)
		return 3;
		
	if(ChangeMemberRank(pMaster->GetID(), pChangeInfo->Rank) == FALSE)
		return 4;

	GuildChangeMemberRank(pMaster->GetGuildIdx(), pMaster->GetID(), pChangeInfo->Rank);

	CPlayer* pMember = (CPlayer*)g_pUserTable->FindUser(pMaster->GetID());
	if(pMember)
	{
		pMember->SetGuildMemberRank(pChangeInfo->Rank);
	}
	...
	notify.dwData = pMaster->GetID();
	notify.bData = pChangeInfo->Rank;
	g_Network.Send2AgentServer((char*)&notify, sizeof(notify));

	LogGuild(pMaster->GetID(), pMaster->GetGuildIdx(), eGuildLog_MasterChangeRank, pMaster->GetID(), pChangeInfo->Rank);

	ChangeMemberRank(pChangeIDX, GUILD_MASTER);
	
	GuildChangeMemberRank(pMaster->GetGuildIdx(), pChangeIDX, GUILD_MASTER);

	pMember = (CPlayer*)g_pUserTable->FindUser(pChangeIDX);
	if(pMember)
	{
		pMember->SetGuildMemberRank(GUILD_MASTER);
	}
	m_GuildInfo.MasterIdx = pChangeIDX;
	SafeStrCpy(m_GuildInfo.MasterName,pChangeName,MAX_NAME_LENGTH+1);

	MSG_DWORDBYTE notify1;
	notify1.Category = MP_GUILD;
	notify1.Protocol = MP_GUILD_CHANGERANK_NOTIFY_TO_MAP;
	notify1.dwObjectID = pMaster->GetGuildIdx();
	notify1.dwData = pChangeIDX;
	notify1.bData = GUILD_MASTER;
	g_Network.Send2AgentServer((char*)&notify1, sizeof(notify1));

	LogGuild(pChangeIDX, pMaster->GetGuildIdx(), eGuildLog_MasterChangeRank, pChangeIDX, GUILD_MASTER);
