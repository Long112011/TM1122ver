...
void CQuickItem::Render()
{
	...
	if(m_pItemLink)
	{
		...
		{		
			static char nums[5];
			wsprintf(nums,"%4d", m_QDur);
			RECT rect={(LONG)m_absPos.x+13, (LONG)m_absPos.y+29, 1,1};
			CFONT_OBJ->RenderNoticeMsg(0,nums,strlen(nums),&rect,RGBA_MERGE(m_dwImageRGB, m_alpha * m_dwOptionAlpha / 100),RGBA_MERGE( RGB_HALF(25 ,25 ,25), 180 ));
		}
		...
		if(GetIconKind( m_pItemLink->GetItemIdx()) == eITEM_ICON)
		{
			...
			if( pItem )
			{
				...
				else if( PETMGR->IsCurPetSummonItem(pItem->GetItemBaseInfo()->dwDBIdx) )
				{
					//cDialog::Render();
					...
					//CFONT_OBJ->RenderNoticeMsg(0,CHATMGR->GetChatMsg(1246),strlen(CHATMGR->GetChatMsg(1246)),&rect,RGBA_MAKE(180,200,255,255),RGBA_MAKE(10,10,10,100));
				...
				else if (pItem->GetItemKind() == eYOUNGYAK_ITEM)  //[³ÔÒ©»ØÆøäÖÈ¾][2017/11/27]
				{
				...
				/*else if( pItem->GetItemKind() == eYOUNGYAK_ITEM)  
				{
					DWORD dwRemainTime = 0;
					float delayRate = HERO->GetDelayGroup()->CheckYoungYakPotionDelay(&dwRemainTime );
					if( delayRate > 0.0f )
					{
						VECTOR2 vScale = { 1.0f, delayRate };
						VECTOR2 pos = { m_absPos.x, m_absPos.y + m_height*(1 - delayRate) };
