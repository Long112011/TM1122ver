...
struct BASE_PET_LIST
{
	...
	WORD	BuffList[MAX_PET_GRADE][6];//刀哥  3pet
#else
...
struct ITEM_GOLD_SHOP
{
	...
	WORD	ItemLimit;
	WORD	ItemLimitKind;
	MONEYTYPE	ItemBuyPrice;
	MONEYTYPE	ItemOfferPrice;
    WORD	ItemBuyType;
	DWORD BuyRight;
	DWORD LimitKind;
	DWORD LimitCount;
...
struct ITEM_GOLD_SHOP
{
	...
	DWORD ItemGoldPrice;
	DWORD ItemMallPrice;
	DWORD ItemMoneyPrice;
	DWORD ItemCount;
};
#endif
struct ITEM_PACKET_CLEAR
{
	WORD  Idx;
	DWORD ItemIdx;
	WORD  IsClear;
...
struct GAMEDESC_INI
{
	GAMEDESC_INI()
	{
		strcpy(DistributeServerIP,"127.0.0.1");
	...
	char splashBitmap[128];
};
struct SEVERLIST
{
	char	DistributeIP[64];
	WORD	DistributePort;
	char	ServerName[64];
	WORD	ServerNo;
	BOOL	bEnter;
