...
CSearchComboBox::CSearchComboBox(CWnd* pParent)
{
	m_pParent = pParent;
	m_bSearchOK = TRUE;
}
...
void CSearchComboBoxEdit::OnLButtonDown(UINT nFlags, CPoint point)
{
	//SetFocus();
	if (((theDlg*)GetParent())->m_QuickFind)
		((theDlg*)GetParent())->m_QuickFind->Invalidate();
	//TT
	CSearchComboBoxEditBase::OnLButtonDown(nFlags, point);
}

void CSearchComboBoxEdit::OnRButtonDown(UINT nFlags, CPoint point)
{
	//SetFocus();
	if (((theDlg*)GetParent())->m_QuickFind)
		((theDlg*)GetParent())->m_QuickFind->Invalidate();
	//TT
	CSearchComboBoxEditBase::OnRButtonDown(nFlags, point);
}
