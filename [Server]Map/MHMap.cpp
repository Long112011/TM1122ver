			...
			if( strcmp( buf, "$MAPNUM" ) == 0 )
			{
				...
				if( buf[0] == '{' )
				{
...

					{
						...
						{
							m_msDesc.bPKAllow = true;//file.GetBool();
						}
...
BOOL CMHMap::NonCollisionLine(VECTOR3* pStart, VECTOR3* pEnd, VECTOR3* Target, CObject* pObject)
{
	if (m_pTileManager == NULL)
		return FALSE;

	return m_pTileManager->CollisionLine(pStart, pEnd, Target, pObject);// m_pTileManager->NonCollisionLine(pStart, pEnd, Target, pObject);
}
