...
BOOL COBalloonName::Render(LONG absX, LONG absY)
{
	...
	if (*m_szMarryName != 0)
	{
		...
		//CFONT_OBJ->RenderFont(m_wFontIdx, m_szMarryName, nSize, &marryrect, RGBA_MERGE(RGB_HALF(255, 0, 255), 180));
		//marryrect.left -= 1;
		//marryrect.top -= 1;
	...
	if (*m_szShiTuName != 0)
	{
		...
		//CFONT_OBJ->RenderFont(m_wFontIdx, m_szShiTuName, nSize, &shiturect, RGBA_MERGE(RGB_HALF(216, 96, 40), 180));
		//shiturect.left -= 1;
		//shiturect.top -= 1;
