	...
	case MP_CHEAT_PET_FRIENDSHIP_SYN:
		{
			...
			for (int i = 0; i < 3; ++i)//����  3pet
			{
				...
				Msg.Category = MP_CHEAT;
				DWORD rt = 0;
				if (rt = pPlayer->GetPetManager()->SetSommonPetFriendship(pPet, pmsg->dwData, TRUE))//����  3pet
				{
	...
	case MP_CHEAT_PET_SKILLREADY_SYN:
		{
			...
			for (int i = 0; i < 3; ++i)//����  3pet
			{
				...
				pPlayer->GetPetManager()->SetSommonPetSkillReady(pPet);//����  3pet
			}
#else
			CPet* pPet = pPlayer->GetPetManager()->GetCurSummonPet();
			if(!pPet)	return;
...
void Move_PetOneTarget(char* pMsg)
{
	...
	if (pPlayer->GetInited() == FALSE || pPlayer->GetInitedGrid() == FALSE)
		return;
#ifdef  _MUTIPET_
	CPet* pPet = (CPet*)g_pUserTable->FindUser(pmsg->dwMoverID);//����  3pet
#else
