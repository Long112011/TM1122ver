...
void CPetManager::ReleaseCurPetMove()
{
	for (int i = 0; i < 3; ++i)
	{
		CCharMove::ReleaseMove(m_pCurSummonPet[i]);
	}
}
...
{//펫 삭제시
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		{
			if (m_pCurSummonPet[i]->GetPetSummonItemDBIdx() == ItemDBIdx)
			{
				RemovePet(m_pCurSummonPet[i]);
			}
		}
...
BOOL CPetManager::UpGradePet(DWORD dwSummonItemDBIdx, BOOL bCheckProb /*= TRUE*/)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		if (m_pCurSummonPet[i])
		{
			if (m_pCurSummonPet[i]->GetPetSummonItemDBIdx() == dwSummonItemDBIdx)
				return eUpgradeFailforSamePetSummoned;
		}
	}
	...
	if (rt == eUpgradeFailforProb)
	{
		...
		LogPet(m_pPlayer->GetID(), m_pPlayer->GetID(), ePetLog_UpGradeFailed, pInfo->PetDBIdx, pInfo->PetSummonItemDBIdx,
			pInfo->PetGrade, 0, pInfo->PetFriendly, pInfo->bAlive);
	}
	else if (rt == eUpgradeSucess)
	{
		...
		LogPet(m_pPlayer->GetID(), m_pPlayer->GetID(), ePetLog_GradeUp, pInfo->PetDBIdx, pInfo->PetSummonItemDBIdx,
			pInfo->PetGrade, 0, pInfo->PetFriendly, pInfo->bAlive);
	}
...
BOOL CPetManager::RevivalPet(DWORD dwSummonItemDBIdx, int iGrade)
{
	...
	LogPet(m_pPlayer->GetID(), m_pPlayer->GetID(), ePetLog_PetRevival, pInfo->PetDBIdx, pInfo->PetSummonItemDBIdx,
...
BOOL CPetManager::CheckBeforeSummonPet(DWORD dwItemDBIdx)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		if (m_pCurSummonPet[i])
		{
			if (m_pCurSummonPet[i]->GetPetSummonItemDBIdx() == dwItemDBIdx)
			{
				pPet = m_pCurSummonPet[i];
				break;
			}
		}
	}
	if (pPet)
	{
		if (pPet->GetPetSummonItemDBIdx() == dwItemDBIdx)
		{
			//봉인 한다.
			SealPet(pPet);
			return FALSE;
		}
		else if (pPet->GetPetKind() == CRISTMAS_EVENTPET)
		{
			SealPet(pPet);
			return TRUE;
		}
...
void CPetManager::SummonEventPet()
{
	...
	SetCurSummonPetKind(pPet, ePK_EventPet);
...
void	CPetManager::SetCurSummonPetKind(CPet* pPet, WORD kind)
{
...
void CPetManager::CheckEventPetSummonRemainTime(CPet* pPet)
{
...
void CPetManager::SealPet(CPet* pPet)
{
	...
	msg.dwData = pPet->GetID();
	PACKEDDATA_OBJ->QuickSend(m_pPlayer, &msg, sizeof(msg));
	SetCurSummonPetKind(pPet, ePK_None);
	g_pServerSystem->RemovePet(pPet->GetID());
...
void CPetManager::ExchangePet(DWORD dwItemDBIdx, CPlayer* pNewMaster, BOOL bReduceFriendship/* = TRUE*/, BOOL bBusiness)
{
	PET_TOTALINFO* pPetInfo = GetPetTotalInfo(dwItemDBIdx);
	...
	LogPet(pNewMaster->GetID(), pNewMaster->GetID(), ePetLog_Exchange, pPetInfo->PetDBIdx, pPetInfo->PetSummonItemDBIdx,
		pPetInfo->PetGrade, pPetInfo->PetStamina, pPetInfo->PetFriendly, pPetInfo->bAlive);

	//2024쾨3墩1휑00:37:59
	if (bBusiness == TRUE)
		pNewMaster->GetPetManager()->AddPetTotalInfo(pPetInfo, eServerBusiness);
	else
...
void CPetManager::PetProcess()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		if (m_pCurSummonPet[i])
		{
			CheckPosValidDistWithMaster(m_pCurSummonPet[i]);

			m_pCurSummonPet[i]->Process();
			CCharMove::MoveProcess(m_pCurSummonPet[i]);
			CalcPetSkillRecharge(m_pCurSummonPet[i]);
			//CheckStaminaZero();
			CheckDelayRelease(m_pCurSummonPet[i]);
			if (CheckCurSummonPetKindIs(m_pCurSummonPet[i], ePK_EventPet))
			{
				CheckEventPetSummonRemainTime(m_pCurSummonPet[i]);
			}
		}
	}
...
CPet* CPetManager::GetPetFromSummonItemDBIdx(DWORD dwItemDBIdx)
{
	for (int i = 0; i < 3; ++i)
	{
		if (!m_pCurSummonPet[i])	continue;
		if (m_pCurSummonPet[i]->GetPetSummonItemDBIdx() == dwItemDBIdx)
		{
			return m_pCurSummonPet[i];
		}
	}
	return NULL;
}
void CPetManager::CheckPosValidDistWithMaster(CPet* pPet)
{
	if (!pPet)	return;
#define PET_VALID_DISTANCE_CHECK_TIME_MSEC	3000
	...
	*/
	...
	VECTOR3* pPetCurPos = CCharMove::GetPosition(pPet);
	...
	{
		//warp
		CCharMove::SetPosition(pPet, pPlayerCurPos);

		CCharMove::EndMove(pPet, gCurTime, pPlayerCurPos);
...
void CPetManager::CalcPetSkillRecharge(CPet* pPet)
{
	//!!!휴식 상태면 홀딩.
// 	if(m_pCurSummonPet->IsPetRest())
// 		return;
	...
	const BASE_PET_LIST* pPetBaseInfo = pPet->GetPetBaseInfo();

	//스태미너가 있으면 게이지 채워주고
	if (1/*FALSE == pPet->IsPetStaminaZero()*/)
	{
...
void CPetManager::CheckDelayRelease(CPet* pPet)	//펫 죽음시. 봉인시 아님.
{
...
void CPetManager::NetworkMsgParse(WORD Protocol, void* pMsg)
{
	...
	case MP_PET_USE_SKILL_SYN:
	{
		...
		if (m_pCurSummonPet[0] && m_pCurSummonPet[0]->UsePetSkill())
		{
			//스킬 게이지 초기화 & 레디상태 초기화
			m_dwSkillRechargeAmount = 0;
			m_bSkillGuageFull = FALSE;
	...
	case MP_PET_SEAL_SYN:
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			if (m_pCurSummonPet[i])
			{
				if (m_pCurSummonPet[i]->GetPetSummonItemDBIdx() == pmsg->dwData)
				{
					pPet = m_pCurSummonPet[i];
					break;
				}
	...
	case MP_PET_MixSyn:
	{
		MSG_DWORD8* pmsg = (MSG_DWORD8*)pMsg;
		MSG_DWORD3 msg;
		msg.Category = MP_PET;
		msg.Protocol = MP_PET_MixAck;
		msg.dwObjectID = m_pPlayer->GetID();
		msg.dwData1 = ITEMMGR->PetMix(m_pPlayer, pmsg, &msg.dwData2);
		m_pPlayer->SendMsg(&msg, sizeof(msg));
		return;
	}
	break;
...
void CPetManager::UpdateCurPetInfo(CPet* pPet, BYTE	bSummonning)
{
	if (!pPet) return;

	//펫 정보 갱신
	pPet->SetPetSummonning(bSummonning);
	const PET_TOTALINFO* pCurPetInfo = pPet->GetPetTotalInfo();
...
int CPetManager::SetSommonPetFriendship(CPet* pPet, DWORD dwFriendship, BYTE	bSummonning)
{
	...
	{
		pPet->SetPetFriendShip(dwFriendship);
		return 0;
	}

	UpdateCurPetInfo(pPet, bSummonning);
}

void CPetManager::SetSommonPetSkillReady(CPet* pPet)
{
...
void CPetManager::SetPetBuffInfo()
{
	...
	for (int j = 0; j < 3; ++j)
	{
		...
		{
			pBaseInfo = m_pCurSummonPet[j]->GetPetBaseInfo();

			for (int i = 0; i < 6; ++i)	//3 : Pet Max BuffNum.
			{
				BuffIdx = pBaseInfo->BuffList[m_pCurSummonPet[j]->GetPetCurGrade() - 1][i];
				...
				{
					float BuffVal = pList->BuffValueData;
...
DWORD CPetManager::GetPetBuffValue(int BuffKind)
{
	DWORD val = m_BuffData[BuffKind].BuffValueData;
	return val;
}
...
void CPetManager::RefleshPetMaintainBuff(player_calc_stats* item_stats)
{
	...
	SetPetBuffInfo();
	m_pPlayer->GetPetManager()->GetPetBuffResultRt(ePB_MasterAllStatUp, item_stats);
	//	m_pPlayer->CalcState();
}
...
void CPetManager::SetPetSummonning(int flag)
{
	for (int i = 0; i < 3; ++i)
	{
		...
		{
			m_pCurSummonPet[i]->SetPetSummonning(FALSE);
		}
		else
		{
			m_pCurSummonPet[i]->SetPetSummonning(TRUE);
		}
	}
...
void CPetManager::CheckSummonningPet()
{
	...
	CItemSlot* pSlot = m_pPlayer->GetSlot(eItemTable_ShopInven);
	while (pInfo = m_PetInfoList.GetData())
	{
		if (m_pPlayer)
		{
			if (TRUE == pInfo->bSummonning && pInfo->PetMasterUserID == m_pPlayer->GetUserID())
			{
				if (IsCurPetFull())
				{
					break;
				}
				ITEMBASE* pPetlItem = pSlot->GetItemInfoDbid(pInfo->PetSummonItemDBIdx);
				if (!pPetlItem)
					continue;
...
void CPetManager::ReducePetFriendshipWithMastersRevive()
{
	for (int i = 0; i < 3; ++i)
	{
		...
		m_pCurSummonPet[i]->AddFriendship(m_iFriendshipReduceAmount, TRUE);

		const PET_TOTALINFO* pInfo = m_pCurSummonPet[i]->GetPetTotalInfo();
		LogPet(m_pPlayer->GetID(), m_pPlayer->GetID(), ePetLog_MasterDie, pInfo->PetDBIdx, pInfo->PetSummonItemDBIdx,
			pInfo->PetGrade, pInfo->PetStamina, pInfo->PetFriendly, pInfo->bAlive);
	}
}
