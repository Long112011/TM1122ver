...
void CMonster::AddChat(char * str)
{
	...
	PACKEDDATA_OBJ->QuickSend(this,&pmsg,pmsg.GetMsgLength());	
}
void CMonster::AddSpeech( DWORD SpeechType, DWORD SpeechIdx )
{
	...
	if( SpeechType == eMSpch_Balloon || SpeechType == eMSpch_BallonChat )	
	{
	...
	else if( SpeechType == eMSpch_SmallShout )	
	{
		msg.dwData1 = SpeechType;
		msg.dwData2 = SpeechIdx;
		msg.dwData3 = GetID();
...
void CMonster::DoDie(CObject* pAttacker)
{
	...
	if( pAttacker->GetObjectKind() == eObjectKind_Player )
	{
		...
		if(pPlayer)
		{
			pPlayer->CountBeforeFame(this);

			if (g_pServerSystem->GetMap()->IsMapKind(eFubenMap))
			DungeonMGR->CheckKillCount(pPlayer,1,m_MonsterInfo.MonsterKind);
		}
	}
