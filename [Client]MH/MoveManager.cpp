...
void CMoveManager::StartMoveEx(CObject* pObject, DWORD CurTime, VECTOR3 * pTargetPos)
{
	...
	/*if (pObject->GetState() == eObjectState_SkillSyn)//luna
	{
		return; //pObject->SetState(eObjectState_Move);
	}
...
{
	//CHATMGR->AddMsg(CTC_SYSMSG, "BOOL");
	//HeroMoveStop();
	...
	if (pHero->GetState() == eObjectState_Engrave)
		return ;
	...
	//	pHero->m_WayPointManager->SetFlagDestination(*pPos);
	}
	if (count == 0)
		return;
	else if (count == 1)
	...
	{
		BOOL t_CollisionCheck_Oneline = MAP->CollisionCheck_OneLine_New(&pHero->m_MoveInfo.CurPosition, pPos);
		...
		else
		{
			...
			BOOL t_PathFindResult = MAP->PathFind(&pHero->m_MoveInfo.CurPosition, pPos, pHero->m_WayPointManager, TRUE);
			...
			//pHero->m_WayPointManager->SetFlagDestination(*pPos);
			if (!AUTOPATH->IsRunning())
			AUTOPATH->SetTargetPos(*pPos, TRUE);
...
{
	//CHATMGR->AddMsg(CTC_SYSMSG, "biubiubiu float");
	//HeroMoveStop();
	//m_bSetMove = FALSE;
	CHero* pHero = OBJECTMGR->GetHero();
	if (/*pHero->IsPKMode() &&*/ SAFEAREAMGR->CheckSafeArea(pPos))
	...
	if (pHero->GetState() == eObjectState_Engrave)
		return FALSE;
	if (pHero->GetState() == eObjectState_Deal)
		return FALSE;
	if(pHero->IsInSpecialState(eSpecialState_Stun))
		return FALSE;
	if( pHero->GetState() != eObjectState_None &&
	...
	//MAP->SetRenderTileData(MAP->CollisionCheck_OneTile(pPos));
	//if (MAP->CollisionCheck_OneTile( pPos ) == FALSE)
	//{
		//return pHero->Move_Simple(pPos);
		//if( MAP->CollisionLine( &t_StartPos, pPos, pPos, MAP->GetMapNum(), pHero ) == FALSE )
			//return FALSE;
	//}

	BOOL t_CollisionCheck_Oneline = MAP->CollisionCheck_OneLine_New(&pHero->GetPosition(), pPos);
	if ( t_CollisionCheck_Oneline == TRUE)
	{
		m_bMoveCount = 0;
	...
	else
	{
		...
		BOOL t_PathFindResult = MAP->PathFind(&pHero->GetPosition(), pPos, pHero->m_WayPointManager, TRUE);
		if (t_PathFindResult = FALSE)
		{
...
void CMoveManager::SetPetTarget( MOVE_INFO* pHeroMoveInfo, WORD wPosCount )
{
	...
	if( 1 == wPosCount )
	{
		...
		if( DEFAULT_PET_FOLLOW_DIST/3 < dist )
		{
	...
	dist = CalcDistanceXZ(&pPet->m_MoveInfo.CurPosition, pHeroLastPos);
 	if( DEFAULT_PET_FOLLOW_DIST/3 < Indist || DEFAULT_PET_FOLLOW_DIST/3 < dist)
 	{
...
BOOL CMoveManager::GetPetLastPos( VECTOR3* pSrcPos, VECTOR3* pFromPos, VECTOR3* pToPos )
{
	...
	if(dist < DEFAULT_PET_FOLLOW_DIST/3)
	{
		////ASSERT(0);
		//printf("//ASSERT %f\n",dist);
		return FALSE;
	}
	pSrcPos->x = pFromPos->x + (Pos.x / dist * DEFAULT_PET_FOLLOW_DIST/3);
	pSrcPos->z = pFromPos->z + (Pos.z / dist * DEFAULT_PET_FOLLOW_DIST/3);
	return TRUE;
}
...
void CMoveManager::SetHeroActionMove(CActionTarget* pTarget,CAction* pAction)
{
	...
	if(pTargetPos == NULL)
		return;
	//if (SetHeroTarget(pTargetPos, 1.f) == FALSE)//luna
	//	return;
...
{
	CHero* pHero = OBJECTMGR->GetHero();
	if(pHero->m_MoveInfo.bMoving == FALSE)//luna not use this
		return;
	CalcPositionEx(pHero,gCurTime);
...
void CMoveManager::PetMoveStop()
{
	...
	if (!OBJECTSTATEMGR->StartObjectState(pPet, eObjectState_None))
		return;

	if( FALSE == pPet->m_MoveInfo.bMoving )//luna not use this
		return;
...
void CMoveManager::SetWalkMode(CObject* pObject)
{
	...
	if(pObject->GetObjectKind() == eObjectKind_Player)
	{
		CPlayer* pPlayer = (CPlayer*)pObject;//luna
...
{
	if (HERO->GetMoveDebugInfo())
	{
	//	CHATMGR->AddMsg(CTC_SYSMSG, "EffectMove 1");
	}
	MOVE_INFO* pMoveInfo = &pObject->m_MoveInfo;
	if(pObject->m_bIsYMoving)
	{
	...
	if (HERO->GetMoveDebugInfo())
	{
	//	CHATMGR->AddMsg(CTC_SYSMSG, "EffectMove 2");
	}
}
...
void CMoveManager::ToggleHeroMoveMode()
{
	...
	if(HERO->m_MoveInfo.MoveMode == eMoveMode_Walk)
	{
		...
		if(GAMEIN->GetCharStateDialog()) GAMEIN->GetCharStateDialog()->SetMoveMode( FALSE );
			
	}
	else
	{
		msg.Protocol = MP_MOVE_WALKMODE;
		if(GAMEIN->GetCharStateDialog()) GAMEIN->GetCharStateDialog()->SetMoveMode( TRUE );
	...
	{
		HERO->SetSkipKyungong(!true);
		HERO->Move_Simple(HERO->m_MoveInfo.GetTargetPosition(HERO->m_MoveInfo.GetCurTargetPosIdx()));		
	}	
}
...
void CMoveManager::NetworkMsgParse(BYTE Protocol,void* pMsg)
{
	...
	case MP_MOVE_CORRECTION:
		{	
			...
			//if( HERO &&	pObject->GetID() == HEROID )//kiv
			//{
			//	HERO->ClearWayPoint();//kiv
			//}
		}
		break;
