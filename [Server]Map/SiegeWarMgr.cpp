	...
	case MP_SIEGEWAR_BATTLEJOIN_SYN:
		{
			...
			CharacterItemStoneOptionInfo(pmsg->dwObjectID, MP_USERCONN_GAMEIN_SYN);//JACK
			...
			CharacterSkinInfo(pPlayer->GetID());//JACK
	...
	case MP_SIEGEWAR_CHEAT:		// Cheat
		{
			...
			case 5:
				{
					if(m_pBattle)// by jack
					if (g_pServerSystem->GetMapNum() == GetSiegeMapNum())// by jack
						m_pBattle->ReturnToMapAllPlayer();
				}
				break;
			case 6:
