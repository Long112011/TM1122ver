...
BOOL CMoveManager::SetHeroTarget(VECTOR3* pPos,float biubiubiu)
{
	...
	else
	{
		...
		BOOL t_PathFindResult = MAP->PathFind(&t_StartPos, pPos, pHero->m_WayPointManager, TRUE);
		if (t_PathFindResult = FALSE)
		{
			return FALSE;
		}
...
//-------------------------------------------------------------
	...
	//	return;
	//}
	//TargetPos->x = (int(TargetPos->x / TILECOLLISON_DETAIL) + 0.5f) * TILECOLLISON_DETAIL;
	//TargetPos->y = 0;
	//TargetPos->z = (int(TargetPos->z / TILECOLLISON_DETAIL) + 0.5f) * TILECOLLISON_DETAIL;
//-------------------------------------------------------------	
	pObject->m_MoveInfo.SetCurTargetPosIdx(0);
	pObject->m_MoveInfo.SetMaxTargetPosIdx(1);

	//VECTOR3 l_v3StartPos = pObject->GetPosition();
	//VECTOR3 l_v3CorrectedTargetPos = *TargetPos;
	//MAP->PathFind_Line(&l_v3StartPos, TargetPos, &l_v3CorrectedTargetPos, HERO);
	...
	if(pObject->IsDied() == FALSE)
	{
		...
		msg.dwMoverID = HEROID;
		msg.cpos.Compress(TargetPos/*&l_v3CorrectedTargetPos*/);
		NETWORK->Send(&msg,sizeof(msg));	
	}
}
