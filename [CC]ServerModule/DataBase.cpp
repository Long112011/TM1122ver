...
#define LOGINDBID				"sa"
#define LOGINDBPASSWORD			"jmPJ8a5yOnssGXlHfc01CeSRcfr8wqWJ"
#define LOGDBNAME				"MHLog"
#define LOGDBID					LOGINDBID//"sa"
#define LOGDBPASSWORD			LOGINDBPASSWORD//"U4lnyXOu#noC0DIN1lFl%oeE5*MXncZb"
#define LOGINGAMENAME			"MHGame"
#define LOGINGAMEDBID			LOGINDBID//"sa"
#define LOGINGAMEDBPASSWORD		LOGINDBPASSWORD//"U4lnyXOu#noC0DIN1lFl%oeE5*MXncZb"
CDataBase g_DB;
...
libRt:
	MessageBox(NULL,"DBTHREAD.dll¼ÓÔØÊ§°Ü","´íÎó",NULL);
	return FALSE;*/
	HRESULT hr = CoCreateInstance(CLSID_DBTHREAD_DLL, NULL, CLSCTX_INPROC_SERVER, IID_DBTHREAD_DLL, (void**)&m_pIDB);
	if (FAILED(hr))
	{
		MessageBox(NULL,"Failed to create IID_DBTHREAD_DLL: component not registered"," error",NULL);
		return FALSE;
	}
	...
	if (!ret)
	{
		...
		LPSTR pmsg = "Could not initialize database class";
		WriteMessage(pmsg);
		return FALSE;
	} 
#ifndef _MONITORINGSERVER
...
BOOL CDataBase::Init(DWORD MaxDBThread,DWORD MaxSameTimeQuery,BOOL bUsingLoginDB)
{
	...
	strcpy(desc.DBName, LOGINGAMENAME);
	strcpy(desc.Default_User_account, LOGINGAMEDBID);
	strcpy(desc.Default_User_guid, LOGINGAMEDBID);
	strcpy(desc.Default_User_pw, LOGINGAMEDBPASSWORD);
