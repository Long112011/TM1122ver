...
void CCharChangeDlg::SetActive( BOOL val )
{
	...
	{
		CAMERA->SetCameraMode(eCM_JackView);
		OBJECTMGR->HideAllNpc(1);
		OBJECTMGR->HideAllPet(!1);
		OBJECTMGR->HideAllMonster(1);
...
void CCharChangeDlg::ChangeFaceType( BOOL bPrev )
{
	...
	if( bPrev )
	{
		if( m_CharacterInfo.FaceType == 0 )
			m_CharacterInfo.FaceType = 7;
		else
			--m_CharacterInfo.FaceType;		
	}
	else
	{
		if( m_CharacterInfo.FaceType == 7 )
...
void CCharChangeDlg::CharacterChangeSyn()
{
	...
	{
		CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(2081));
		return;
	}
