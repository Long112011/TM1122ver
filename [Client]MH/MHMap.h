...
public:
	...
	void SetFieldView(float FovVal){mapDesc->Fov=FovVal;}
	void ApplyMapDesc();
...
public:
	...
	void RenderTile2D();
	void RenderOneTile(const VECTOR3& p_TileOriPos, const DWORD p_Color, BOOL mode = FALSE);
	void RenderOneWayPoint(const VECTOR3& p_WayPointPos, DWORD p_Color);
	...
	float GetMapFOV(){return mapDesc->Fov;}
	float GetMapDefaultSight(){return mapDesc->DefaultSight;}

	void TestUji();
	void RenderBoldOneTile(VECTOR2 f_2DPos1, VECTOR2 f_2DPos2, BOOL ForBigMap, DWORD color);

	std::deque<VECTOR3> GetPath_Optimize_Debug(){ return m_Path_Optimize_Debug; }
};
