...
{	
	...
	{
		MAP->InitMap(sCharSelect.wMapNum);//MAP->InitMap(103);
	}
	else
	{
		MAP->Release();
	...
	if(REDISCONN->GetStart())
	{
		...
		if(m_pReconnectDlg)
		{
			m_pReconnectDlg->AddMessage(IMAGEMSG, 1348, (eReconnectDlg)BUTTON);//m_pReconnectDlg->AddMsgFromString(FALSE,"testing this WITH cancel");
		}
	}
...
void CCharSelect::Process()
{
	...
	else if(KEYBOARD->GetKeyPressed(KEY_PADENTER)||KEYBOARD->GetKeyPressed(KEY_RETURN))
	{
		if( GetCurSelectedPlayer()==NULL )
		{
			for( int i = 0 ; i < MAX_CHARACTER_NUM ; ++i )
			{
					SelectPlayer( 0 );
					SetCurSelctedPlayer(0);
					break;
			}
		}
		else
			...
			case MP_USERCONN_CHARACTERLIST_ACK:
				{	
					...
					for(int i = 0; i < pmsg->CharNum ; i++)
					{
						...
						pPlayer->SetAngle(sCharSelect.vAngle[standpoint]);//pPlayer->SetAngle((standpoint * -0.15f));
						...
						pPlayer->GetEngineObject()->EnableShadow(false);						
					}
					InitCameraInCharSelect();
				}				
				return;
			...
			case MP_USERCONN_CHANNELINFO_ACK:
				{
					...
					if(pmsg->Count == 1)
					{
						GetChannelDialog()->SetActive(TRUE);
					}
				}
				return;
...
BOOL CCharSelect::EnterGame()
{
	...
	loadingParam.selectedCharNum = GetCurSelectedPlayer()->GetID();
	loadingParam.m_MapNum = GetCurSelectedPlayer()->GetCharacterTotalInfo()->LoginMapNum;

	MAINGAME->SetGameState(eGAMESTATE_GAMELOADING, LPVOID(&loadingParam), sizeof(GameLoadingParam));
