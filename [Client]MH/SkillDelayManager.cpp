...
DWORD CSkillDelayManager::GetSkillDelay(DWORD dwSkillIndex)
{
	PRIMERESKILL* pPrimereSkill = m_htPrimereSkill.GetData(dwSkillIndex);
	if(pPrimereSkill)
		return pPrimereSkill->dwDelay;
	else
		return 0;
}
BOOL CSkillDelayManager::CheckAdaTak(DWORD dwSkillIndex)
{
	PRIMERESKILL* pPrimereSkill = m_htPrimereSkill.GetData( dwSkillIndex );
	if(pPrimereSkill)
		return pPrimereSkill->dwSkillIndex;
	else
		return FALSE;
}