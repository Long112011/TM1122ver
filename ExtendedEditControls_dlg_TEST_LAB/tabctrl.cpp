...
BEGIN_MESSAGE_MAP(TabControl, CTabCtrl)
	...
	ON_WM_CONTEXTMENU()
	ON_COMMAND(IDR_COLOREDITCTX, OnNewFolder)
	//WM_CONTEXTMENU
END_MESSAGE_MAP()

void TabControl::OnNewFolder()
{
	//unsigned int nSelectedTab = GetCurSel();
	//DeleteItem(nSelectedTab);

	NMHDR nmh;
	nmh.hwndFrom = GetSafeHwnd();
	nmh.idFrom = IDC_CONTEXT;// GetDlgCtrlID();
...
void TabControl::OnContextMenu(CWnd* pWnd, CPoint point)
{
	SetFocus();
	...
	//for (int i = 1; i < 100; ++i)
	//{
	s.Format(_T("Close file(experimental)"));
	menu.AppendMenu(MF_STRING, IDR_COLOREDITCTX, s);
	//}
	MENUINFO mi = { sizeof(MENUINFO) };
	mi.fMask = MIM_MAXHEIGHT;
	mi.cyMax = 400;
	menu.SetMenuInfo(&mi);
	int Cmd = menu.TrackPopupMenu(TPM_RETURNCMD, x, y, this);
	if (Cmd > 0)
	{
		if (Cmd == IDR_COLOREDITCTX)
		{
			pWnd->SendMessage(WM_COMMAND, IDR_COLOREDITCTX);
		}
		else
		{
...
//
TabControl::TabControl()
: m_bRedirect(true), m_bDragging(false), m_InsertPosRect(0, 0, 0, 0), m_pSpinCtrl(0), m_bHotTracking(false)
{
}
...
bool TabControl::ReorderTab(unsigned int nSrcTab, unsigned int nDstTab)
{
	...
	NMHDR nmh;
	nmh.hwndFrom = GetSafeHwnd();
	nmh.idFrom = IDC_TAB;// GetDlgCtrlID();
	nmh.code = TCN_SELCHANGE;
...
//
// @mfunc Overridded WindowProc to take care of redirection of the tab indexes. If also
...
LRESULT TabControl::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		// Set or action messages (redirect tab number before call)
	case TCM_INSERTITEM:
		if (m_bRedirect)
		{
	...
	case TCM_SETITEM:
		...
		// Get messages (redirect tab number after call)
