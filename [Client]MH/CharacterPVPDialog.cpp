...
{
	m_ppStatic.PVPDef = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_PVP_DEFENSE);//PVP防御力
	m_ppStatic.PVPADef = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_PVP_RESIST);//PVP抵抗力
	m_ppStatic.PVPCri = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_PVP_CRITICAL_AND_RESIST);//PVP一击 / 一击抵抗
	m_ppStatic.PVPCriticalDamage = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_PVP_CRITICAL_DAMAGE_INC_AND_DEC);//PVP一击伤害增加 / 减少
	m_ppStatic.PVPADodge = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_PVP_DODGE_AND_ACCURACY);//PVP回避 / 命中
	m_ppStatic.PVPDAMAGE_Des = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_PVP_DAMAGE_INC_AND_DEC);//PVP伤害增加 / 减少
	...
	m_ppStatic.critical = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_CRITICAL_AND_RESIST);//一击 / 一击抵抗
	m_ppStatic.criticaldamage = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_CRITICAL_DAMAGE_INC_AND_DEC);//一击伤害增加/减少
	m_ppStatic.dodge = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_DODGE_AND_ACCURACY);//回避 / 命中
	m_ppStatic.stunpvp = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_STUN_AND_RESIST);// 晕眩 / 晕眩抵抗
	m_ppStatic.phyandattdamage = (cStatic*)GetWindowForID(MY_INFO_PVP_STC_DAMAGE_INC_PHY_AND_ATT);//一击 / 一击抵抗
}
...
void CCharacterPvpDialog::SetAttackRate()//攻击力
{
	char buf1[66];
	DWORD minv, maxv;
...
void CCharacterPvpDialog::SetALLPhyDefense()//天墨技术修改 2023 - 12 - 29 修复65535 极限
{
