...
void CSWProtectRegDlg::Test()
{
	...
	m_pListDlg->AddItem( "抛胶飘而聪促!!", 0xffffffff );
...
void CSWStartTimeDialog::Test()
{
	...
	sprintf( temp, "%02d月 %02d日 %02d点", time1.GetMonth(), time1.GetDay(), time1.GetHour() );
	m_pTime0->SetStaticText( temp );

	sprintf( temp, "%02d月 %02d日 %02d点", time0.GetMonth(), time0.GetDay(), time0.GetHour() );
	m_pTime3->SetStaticText( temp );

	time0 += time;
	sprintf( temp, "%02d月 %02d日 %02d点。", time0.GetMonth(), time0.GetDay()-1, time0.GetHour()+24 );
	m_pTime4->SetStaticText( temp );
