...
#include "stdafx.h"
#include <string>           // Á´òË?string ?æÂ©¶Ë∫áÂü•?ªÈ??öÂ? 2014-12-06
...
// Ë£??ÑÂ•ß Â¨¥Ê???Ëπ∫È? by Stiner(2008/05/29)-MapDropItem
#include "MapItemDrop.h"
#include "SafeArea.h"            //weiye ?áÔüµ?•È??õÊ???2018-03-15
...
void CServerSystem::ReadServerConfig(WORD MapNum)
{
    ...
    ReStagePower =::GetPrivateProfileInt("ReStageConfig","ReStagePower",0,".\\GameConfig.ini");   // Áæ≤Â£Ω 2015-02-15
...
//	STARTLOG(GAMERESRCMNGR->LoadTitanEquipEnduranceInfo());
	...
	ITEMMGR->LoadSetItemOption();
		ITEMMGR->LoadSetItemQualityOption();//º”‘ÿ◊∞±∏∆∑÷ –≈œ¢
...
//	}
	...
	while(pObject = (CObject*)g_pUserTable->GetUserData())
	{
		...
		if(pObject->GetObjectKind() == eObjectKind_Pet )
			continue;	//ËºøÊ™£???ÆÁ??ÆËù∂Á∏ëÊÜÆ Á±Ä?? Ë¨ôÊ???
		...
		if(pObject->GetObjectKind() == eObjectKind_Player)
		{
			...
			//pPlayer->FBProcess();          //weiye FB?µÊ?ËºõÊ?Á°í‰ø¥      2018-02-06
	...
	//db?ΩÁ? ?≥Ëêµ ?≠Ëë¨??Ê∞àÊ£ªË≥?..
	g_DB.ProcessingQueue();	//ProcessingDBMessage()???æÂ????ÖËºøË≥?Ê©´Èõ≤Ê¢?
//	g_DB.ProcessingDBMessage();
	...
	if(g_pAISystem)
		g_pAISystem->Process();
	//MonsterStateProc();		// ÔπåÔ?o?úo?úÔ?AI ?≥ioAA Ôπ†i‰ΩøoÔπåÔ?ÔπåcÔπåÔ?A CO?úuo
	
	LOOTINGMGR->ProcessTimeOutCheck();	//?úoA?≥Ë??≥IAI ÔπåÔ?UÔπ†iE PK?≥Ô?c?ú‚ï≥A ~?≥Ô?aCI?≥Â?a
	QUESTMGR->Process();
...
void CServerSystem::CreateNewPet( CPlayer* pMaster, DWORD dwItemIdx, DWORD SummonItemDBIdx, WORD wPetKind, WORD PetGrade /*= 1*/ )
{
	...
	TotalInfo.PetSummonItemDBIdx = SummonItemDBIdx;			//Â¨¥Ê??±Ê? DBID	//ÁæÖÂ???Ê©æÈôΩ ?ÆÔ?.
	TotalInfo.PetKind		= wPetKind;	//Ë¨ôÁ?
	TotalInfo.PetStamina	= pPetListInfo->StaminaMax[PetGrade-1];	//?∂Á??êÈá≠ Ë≠ÜÊ?Á∫?
	TotalInfo.PetFriendly = GAMERESRCMNGR->GetPetRule()->DefaultFriendship;		//?¶Êô¶?ÑËÄÄÂ°µÁ¥´
	TotalInfo.PetGrade	= PetGrade;	//Ê¨ΩÂ?
	TotalInfo.PetMasterUserID	= pMaster->GetUserID();	//?∑Ë?? ?∑Ë? DBÂ©¶Ê∫º
...
//		SafeStrCpy(BaseInfo.ObjectName, pPetListInfo->Name, MAX_NAME_LENGTH+1);
//
//		PetInfo.PetSummonItemID = SummonItemID;			//Â¨¥Ê??±Ê? DBID	//ÁæÖÂ???Ê©æÈôΩ ?ÆÔ?.
//		PetInfo.PetKind		= wPetKind;	//Ë¨ôÁ?
//		PetInfo.PetStamina	= pPetListInfo->StaminaMax[0];	//?∂Á??êÈá≠ Ë≠ÜÊ?Á∫?
//		PetInfo.PetFriendly = PET_DEFAULT_FRIENDLY;		//?¶Êô¶?ÑËÄÄÂ°µÁ¥´
//		PetInfo.PetGrade	= 1;	//1Ê¨ΩÂ?
//		PetInfo.PetMasterUserID	= pMaster->GetID();	//?∑Ë??
//		PetInfo.bAlive		= TRUE;
//		//PetInfo.MapNum		?∑Ë??
//
...
CMonster* CServerSystem::AddMonster(DWORD dwSubID, BASEOBJECT_INFO* pBaseObjectInfo,MONSTER_TOTALINFO* pTotalInfo,VECTOR3* pPos,WORD wObjectKind)
{
	...
	//2007. 7. 5. CBH - ?™ÂÉ•?¶Á? ?™Ëæ® Ë∑®Ëù∂??Ë¨ôÁ? ?≤Ë? Ëπ∫È?
	...
	//2007. 10. 30. CBH - È°´Ê?È©??™Ëæ® Ë∑®Ëù∂??Ë¨ôÁ? ?≤Ë? Ëπ∫È?
...
CFieldBossMonster* CServerSystem::AddFieldBossMonster(DWORD dwSubID, BASEOBJECT_INFO* pBaseObjectInfo,MONSTER_TOTALINFO* pTotalInfo,VECTOR3* pPos)
{
	...
	//sprintf(msg.Msg,"[%s]?§ËèØ?ûBOSS?§Á?Â©ì[%s] [%d]???¥Ê? %1.0f,%1.0f",pBaseObjectInfo->ObjectName,GetMap()->GetMapName(),pBaseObjectInfo->BattleID,pPos->x / 100,pPos->z / 100);
	//g_Network.Broadcast2AgentServer((char*)&msg,sizeof(msg));
...
CNpc* CServerSystem::AddNpc(BASEOBJECT_INFO* pBaseObjectInfo,NPC_TOTALINFO* pTotalInfo,VECTOR3* pPos,float angle/* = 0*/)
{
	...
	pNpc->SetNpcAngle(angle);	//SW070704 Ëπ∫È?
...
{
	//SetUnableProcess(TRUE);				//1. Process ?¨Ê? & DBReturnMsg Á±Ä??ÂØ∞Ô?.

	//g_Console.LOG(4, "[0/4]Start Working For Map Server Shutdown" );

	RemoveServerForKind(AGENT_SERVER);	//2. AgentSrv ?íÊô¶.
	...
	{	
		if(pObject->GetObjectKind() == eObjectKind_Player)
			...
			//FALSE: Ë§íËñØ??UserTableÁ∏ëÊÜÆ???ñËæ¶??ÂΩäÊ?Ê£?
		else
		{
...
//	FRIENDMGR->UserLogOut(pPlayer->GetID());
	...
	// YH d?úÔ?ÔπåÔ? ?úu?≥AÔπ†i?≥IAI?ú‚ï≥ÔπåcÔπåÔ?A AE?≥Â?aE?≥‰Ωø?≥Ë?ÔπåÔºπ fÔπåÔ?e?≥Ô?I Ôπ†iE /Au?≥Ë?ÔπåÔºπ ‰ΩøÂ??úÔ??≥Ë??≥IÔπåO?≥‚ñºÔπåÔ?ÔπåÔ? C?úË?A?úË?ÔπåÔ?U.
	if(pPlayer->GetInited() == TRUE)
	{
		...
		//Ac?≥iy ?≥Ë?u?≥Ô?A ?≥Ë?‰Ωø‚ï≥CeAÔπåÔºπ, ?≥iyÔπåÔ?i?≥Ô?A, ‰ΩøÂ??≥i?≥Ô?A ?≥Â?iAO. (DataÔπåÔ?| AuAaCI?≥Â?a AuÔπ?ÔπåÔºπ ?≥Ë?‰Ωø‚ï≥CeAÔπåÔºπÔπ?ÔπåÔºπ ÔπåÔ?eCN ?úÔ?I?úÔ??úÔ?; A‰ΩøÂ?ÔπåÔ?ÔπåcC?úË??úu‰ΩøÔ? COÔπ†ii.)
		//ResultÔπ†iC?úui?úu?≥‰Ωø KINE_NONEAI Ôπ†iC?úuuÔπåÔ?UÔπåÔ?e?
		if(pPlayer->GetState() == eObjectState_Die)
		{
...
#endif
			{
				...
				else	//?úÔ?n‰Ωøo?≥iA‰ΩøÔ? A?≥Ô??úuuÔπåÔ?U.	//?úÔ?n‰Ωøo?≥iA‰ΩøÔ? ÔπåÔ?o?úo?úÔ?AIÔπ?ÔπåÔºπ?≥Ë?O A?≥Ô?; ?úuoÔπ†iÔπ†i AOÔπåÔ?U. confirm
				{
					pPlayer->ReviveAfterVimu( FALSE );	//ÔπâaÔπâuA?Ü‚ï±Ôºπ‚ï±??. msgË∂≥‚ï±?Ü‚àÆAo ?ñE?ûA?ûU.6
				}
			}
		}

		// A‰ΩøÂ?ÔπåÔ??d?úÔ?ÔπåÔ? ?úu?≥AÔπ†i?≥IAI?ú‚ï≥Ôπåc///////////////////////
		if( pPlayer->IsPKMode() )
...
//		g_Console.Log(eLogDisplay, 4, "DisconnectUser : %d %s", dwPlayerID, pPlayer->GetObjectName());
	...
	{
		pPlayer->SetFBTimerMonsterNum(0);  //CoNaN Áæ≤Â??µÊ?ËºõÊ??õËÇÆÂ•ÄÁ¨≠Á¶ª‰ºÄÂ¢ÖÊ?Ë¨? 2018-12-15
	...
	UnRegistLoginMapInfo(dwPlayerID); //dbÔπ?ÔπåÔºπ CoAc ÔπåÔ?E‰Ωøo‰ΩøË?E?≥I 08?≥Ô?I ?úuA?ú‚ï≥A
...
void CServerSystem::ProcessWaitExitPlayer()
{
	...
	if( pInfo )
	{
		if( gCurTime - pInfo->dwStartTime >= 5*1000 )	//10??-->5?æÁ? Êª≤Âî≥(2005.11.1)
		{
...
// RaMa -04.11.24
	while( !file.IsEOF() )
	{
		...
		else if (strcmp(Token, "#MUGONGPHY") == 0)//PVPŒÔ¿Ìπ•ª˜
		{
			gEventRateFile[eEvent_MugongPhy] = file.GetFloat();
		}
		else if (strcmp(Token, "#MUGONGATTR") == 0)//PVP Ù–‘π•ª˜
		{
			gEventRateFile[eEvent_MugongAttr] = file.GetFloat();
		}
		else if (strcmp(Token, "#MUGONGKILLMR") == 0)//PVE…À∫¶±∂¬  ’Ω ø∑® ¶π≤”√
		{
			gEventRateFile[eEvent_MugongKillMonster] = file.GetFloat();
		}
		else if (strcmp(Token, "#MUGONGPHYCRI") == 0)//PVP/PVE±©ª˜±∂¬   ’Ω ø
		{
			gEventRateFile[eEvent_MugongPhyCritical] = file.GetFloat();
		}
		else if (strcmp(Token, "#MUGONGATTCRI") == 0)//PVP/PVE±©ª˜±∂¬   ∑® ¶
		{
			gEventRateFile[eEvent_MugongAttrCritical] = file.GetFloat();
		}
		else if (strcmp(Token, "#AssassinPhyDmg") == 0)//¥ÃøÕµƒŒÔ¿Ì…À∫¶ÃÌº”¬ 
		{
