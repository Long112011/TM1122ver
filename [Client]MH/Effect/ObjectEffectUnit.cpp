...
#ifndef _OZMSO_
		if( ( eOO_EFFECT_DEFAULT == dwEffectOption )  )
#else
		if( ( eOO_EFFECT_DEFAULT == dwEffectOption ) ||
			( ( eOO_EFFECT_ONLYHERO == dwEffectOption ) && 
			( pObject->pTarget == (CObject*)HERO || pParam->m_pSkillOperator == (CObject*)HERO || pParam->m_pOperator == (CObject*)HERO )
			) )
#endif
		{
			...
			{
				//ASSERT(0);kiv jack //1750 05032022
			}
			else
			{
