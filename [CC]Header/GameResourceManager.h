...
#define GET_MAINWIN_FULLSCREEN	GAMERESRCMNGR->m_GameDesc.dispInfo.dispType
struct  WIN_DPI
{
	int Index;
	int x;
...
struct sPRELOAD_PAIREFFECT_INFO{
	int EffectRight;
	int EffectLeft;
};
...
class CGameResourceManager  
{
	...
	CYHHashTable<stDungeonKey>		m_DungeonKeyList;
	DWORD m_MaxItemChxCount;
	CHXLIST* m_ItemChxList;
	WORD g_MaxLevel;
	WORD g_MaxGuildMember;

	//WORD g_MaxZColumn;
	WORD g_PortalCountDown;
private:
...
public:
	BOOL LoadMaxLevelControl();
	WORD GetMaxLevelControl(){return g_MaxLevel;}
	WORD GetMaxGuildMember(){return g_MaxGuildMember;}
	//WORD GetMaxZColumn(){return g_MaxZColumn;}

	WORD GetPortalCountDown(){ return g_PortalCountDown; }
	int gCommonSet[eCommonMax];
	BOOL LoadRebornSetting();
	//DWORD GetRebornSet(DWORD dwSet);
	int GetRebornSet(int Rate);
...
#define BOBUSANG_POSNUM_MAX	4	
	...
	BOOL LoadFaceModFileList(MOD_LIST pFaceModList[GENDER_MAX]); 
	...
	void GetDispInfo( DISPLAY_INFO* dispInfo ) { *dispInfo = m_GameDesc.dispInfo; }
	BOOL LoadGameDesc();
	...
	void SetNewResolution(DWORD x,DWORD y,BOOL windowed);
	...
	CYHHashTable<sPRELOAD_PAIREFFECT_INFO>	m_PreLoadNewEffectPair;		
	sPRELOAD_PAIREFFECT_INFO* GetNewPairEffectInfo(WORD wItemType);		
	BOOL LoadNewItemPairEffect();										
	CYHHashTable<NEWITEM_EFFECT_INFO> m_NewItemEffectInfoTable;			
	BOOL LoadNewItemEffect();											
	void RemoveNewItemEffectTable();									
	NEWITEM_EFFECT_INFO* GetNewItemEffectInfo(WORD wItemType);			
	CYHHashTable<WIN_DPI>  m_WindowDpiList;
	BOOL IsFullScreen(){return m_GameDesc.dispInfo.dispType;}
	void PreLoadData();	
	SEVERLIST	m_ServerList[30];
	int			m_nMaxServerList;
	BOOL		LoadServerList();
	char*		GetServerSetName();			
	SEVERLIST * GetServerList(WORD Index);
	BYTE bResolutionIndex;
	BYTE GetResolutionIndex()	{	return bResolutionIndex;	}
	void SetResolutionIndex(int Index){bResolutionIndex=Index;}
	WIN_DPI * GetPrintScressInfo(int ResolutionIndex){ return m_WindowDpiList.GetData(ResolutionIndex); }
